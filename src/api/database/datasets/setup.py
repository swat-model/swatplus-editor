...
class SetupDatasetsDatabase():
	...
	def check_version(datasets_db, editor_version, updateable = False):
		...
		if db_lib.exists_table(conn, 'version'):
			...
			if not (float(ver) >= min_version or m.value == editor_version):
				return 'Please update the swatplus_datasets.sqlite file in your project folder to the most recent version at {download_link}. Your version is {current_version}.'.format(download_link=download_link, current_version=m.value)
		else:
			conn.close()
			return 'Please update the swatplus_datasets.sqlite file in your project folder to the most recent version at {download_link}, before creating your project.'.format(download_link=download_link)
	...
	@staticmethod
	def add_decision_tables(file_path):
		files_dtable.D_table_dtl(file_path).read('datasets')
