	...
	def insert_reservoirs(self):
		...
		if reservoir.Reservoir_res.select().count() == 0:
			...
			if res_query.count() > 0:
				...
				res_rel_id = None
				res_rel = decision_table.D_table_dtl.get_or_none(decision_table.D_table_dtl.name == 'drawdown_days')
				...
				if source_row is not None:
					...
					try:				
						con_outs.append({
							attach_key: id,
							'order': orders[id],
							'obj_typ': cats_to_obj_typ[source_row.sourcecat],
							'obj_id': cats_to_list[source_row.sourcecat][source_row.sourceid],
							'hyd_typ': source_row.hyd_typ,
							'frac': source_row.percent / 100
						})
