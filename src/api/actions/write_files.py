...
class WriteFiles(ExecutableApi):
	...
	def write_climate(self, start_prog, allocated_prog):
		...
		prog += prog_step
		atmo_cli_file = files[8].strip()
		if atmo_cli_file != NULL_FILE:
			self.update_file_status(prog, atmo_cli_file)
			climate.Atmo_cli(os.path.join(self.__dir, atmo_cli_file), self.__version, self.__swat_version).write()
	...
	def write_water_rights(self, start_prog, allocated_prog):
		num_files = 1
		files = self.get_file_names("water_rights", num_files)

		prog_step = round(allocated_prog / num_files)
		prog = start_prog

		water_allocation_wro_file = files[0].strip()
		if water_allocation_wro_file != NULL_FILE:
			self.update_file_status(prog, water_allocation_wro_file)
			water_rights.Water_allocation_wro(os.path.join(self.__dir, water_allocation_wro_file), self.__version, self.__swat_version).write()
	...
	def write_basin(self, start_prog, allocated_prog):
		...
		prog += prog_step
		carbon_lyr_bsn_file = files[3].strip()
		if carbon_lyr_bsn_file != NULL_FILE:
			self.update_file_status(prog, carbon_lyr_bsn_file)
			basin.Carbon_lyr_bsn(os.path.join(self.__dir, carbon_lyr_bsn_file), self.__version, self.__swat_version).write()
