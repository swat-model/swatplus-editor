...
def get_landscape(ls, ncycle, aqu):
	...
	if aqu is not None:
		landscape.nLosses.leached = aqu.no3_seep
		landscape.nLosses.groundwaterYield = aqu.no3_lat
...
def get_landuse():
	...
	urb_ids = [o.id for o in hru_parm_db.Urban_urb.select(hru_parm_db.Urban_urb.id).order_by(hru_parm_db.Urban_urb.id)]
	...
	if len(hru_wb) == len(hru_ls) and len(hru_wb) == len(hru_pw):
		...
		for wb in hru_wb:
			...
			if landuse == 'NA':
				try:
					urb_id = urb_ids[wb.unit - 1]
					urban = hru_parm_db.Urban_urb.get_or_none(hru_parm_db.Urban_urb.id == urb_id)
...
class GetSwatplusCheck(ExecutableApi):
