...
class UpdateProject(ExecutableApi):
		...
		if upgrade_chain is not None:
			...
			for upgrade in upgrade_chain:
				try:
					...
					if upgrade == '1_2_0':
						method(project_db, update_project_values)
					elif upgrade == '1_1_0':
						method(project_db, datasets_db)
					else:
						method(project_db)
	...
	def updates_for_3_0_0(self, project_db):
	...
	def updates_for_2_3_0(self, project_db):
		...
		for harv_op in new_harv_ops:
			...
			if ho is None:
				ops.Harv_ops.insert(name=harv_op['name'], harv_typ=harv_op['harv_typ'], harv_idx=harv_op['harv_idx'], harv_eff=harv_op['harv_eff'], harv_bm_min=harv_op['harv_bm_min']).execute()

	def updates_for_2_1_0(self, project_db):
		self.emit_progress(5, 'Running migrations...')
		migrator = SqliteMigrator(SqliteDatabase(project_db))
		migrate(
	...
	def updates_for_2_0_0(self, project_db):
		self.emit_progress(5, 'Running migrations...')
		migrator = SqliteMigrator(SqliteDatabase(project_db))
		migrate(
	...
	def updates_for_1_3_0(self, project_db):
		self.emit_progress(5, 'Running migrations...')
		migrator = SqliteMigrator(SqliteDatabase(project_db))
		migrate(
	...
	def updates_for_1_2_1(self, project_db):
		lum.Landuse_lum.update({lum.Landuse_lum.plnt_com: None}).where(lum.Landuse_lum.name == 'barr_lum').execute()

	def updates_for_1_2_0(self, project_db, update_project_values):
		migrator = SqliteMigrator(SqliteDatabase(project_db))
		migrate(
	...
	def updates_for_1_1_0(self, project_db, datasets_db):
		conn = lib.open_db(project_db)
