...
<template>
	...
	<v-btn v-else-if="constants.globals.platform === 'win32' && !props.noIcon" @click="open" :active="false">
		<v-icon>fas fa-folder-plus</v-icon> {{ props.text }}
	</v-btn>
	<v-btn
		v-else-if="constants.globals.platform === 'win32' && props.noIcon"
		@click="open"
	...
	<v-dialog v-model="unavailable.show" width="auto">
		<v-card :title="unavailableTitle">
			<v-card-text>
				<p v-if="unavailable.norun" class="py-5">You must run the model before opening your project in SWAT+ IAHRIS.</p>
				<p v-else-if="unavailable.notinstalled" class="py-5">
					SWAT+ IAHRIS is software that links SWAT+ to IAHRIS to automatically generate reports on Indicators of Hydrologic Alteration in
					RIverS.
				</p>
			</v-card-text>
			<v-divider></v-divider>
			<v-card-actions>
				...
				<open-in-browser
					v-else-if="unavailable.notinstalled"
					url="https://swat.tamu.edu/software/plus/"
					text="Install SWAT+ IAHRIS"
					button
					color="primary"
				/>
