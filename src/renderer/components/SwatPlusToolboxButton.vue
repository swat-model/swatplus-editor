...
<template>
	...
	<v-btn v-else-if="!props.noIcon" @click="open" :active="false"> <v-icon>fas fa-toolbox</v-icon> {{ props.text }} </v-btn>
	<v-btn
		v-else-if="props.noIcon"
		@click="open"
	...
	<v-dialog v-model="unavailable.show" width="auto">
		<v-card :title="unavailableTitle">
			<v-card-text>
				<p v-if="unavailable.norun" class="py-5">You must run the model before opening your project in SWAT+ Toolbox.</p>
				<p v-else-if="unavailable.notinstalled" class="py-5">
					SWAT+ Toolbox can be used for sensitivity analysis, model calibration, evaluation, and validation. Click the button below to
					download.
				</p>
			</v-card-text>
			<v-divider></v-divider>
