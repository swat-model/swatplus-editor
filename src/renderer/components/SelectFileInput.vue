...
const emit = defineEmits(["update:modelValue", "change", "blur", "focus", "keyup"]);
...
function selectFile() {
	...
	if (props.fileType == "json") {
		...
		filters = [
			{ name: "Cli/Text", extensions: ["txt", "cli"] },
			{ name: "All files", extensions: ["*"] },
		];
	...
	let files: any;
	if (props.saveDialog) {
		files = electron.saveFileDialog({
			filters: filters,
			defaultPath: props.defaultFileName,
		});
	} else {
...
<template>
	<v-text-field
		...
		append-icon="fas fa-folder-open"
