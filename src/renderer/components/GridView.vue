...
<template>
	<project-container :loading="page.loading">
			...
			<v-table class="data-table" fixed-header :height="autoHeight ? 'auto' : tableHeight" density="compact">
				<thead>
					<tr class="bg-surface">
						...
						<th
							...
							:key="header.key"
							:class="`${header.class} pointer bg-secondary-tonal`"
							@click="doSort(header.key)"
						>
							{{ formatters.isNullOrEmpty(header.label) ? header.key : header.label }}
							<v-icon v-if="!header.noSort && table.sortBy[0] === header.key && table.sortBy[1] === 'asc'" class="fa-xs ms-2"
								>fas fa-arrow-up</v-icon
							>
							<v-icon v-if="!header.noSort && table.sortBy[0] === header.key && table.sortBy[1] === 'desc'" class="fa-xs ms-2"
				...
				<tbody v-if="table.loading">
					<tr v-for="i in loaderArray" :key="i">
						...
						<td v-for="header in table.headers" :key="header.key">
					...
					<tr v-if="!data.items || data.items.length < 1">
						<td :colspan="headerCount" class="text-center text-medium-emphasis py-6">
							<em
								>No data available.
								{{
									!props.hideCreate && formatters.isNullOrEmpty(table.filter)
										? "Use the button at the bottom of this page to create a new record."
										: ""
								}}</em
							>
						</td>
					...
					<tr v-for="(item, i) in data.items" :key="i">
						...
						<td v-for="header in table.headers" :key="header.key" :class="header.class">
							...
							<div v-else-if="header.type === 'object'">
								...
								<router-link
									v-else-if="!formatters.isNullOrEmpty(header.objectRoutePath)"
							...
							<div v-else-if="header.type === 'file'">
								...
								<span v-else>
									<open-file :file-path="`${header.filePath}\\${item[header.key]}`" class="text-primary text-decoration-none">{{
										item[header.key]
									}}</open-file>
								</span>
							</div>
							<div v-else-if="header.type === 'variable-object'">
								<span v-if="formatters.isNullOrEmpty(item[header.key])">-</span>
								<router-link
									v-else-if="utilities.getObjTypeRoute(item) !== '#'"
									class="text-primary text-decoration-none"
							...
							<font-awesome-icon
								...
								@click="
									askDelete(
										item[itemPk],
										item.name,
										formatters.isNullOrEmpty(props.compositeKey) ? '' : item[props.compositeKey].id,
									)
								"
							></font-awesome-icon>
						</td>
		...
		<action-bar v-if="!props.noActionBar" :full-width="props.fullWidthActionBar" :fullest-width="props.fullestWidthActionBar">
			...
			<v-btn
				v-if="data.items && data.items.length > 0 && props.showDeleteAll"
				variant="flat"
				color="error"
				class="mr-2"
				@click="page.deleteAll.show = true"
				>Delete All</v-btn
			>
		...
		<v-dialog v-model="page.deleteAll.show" :max-width="constants.dialogSizes.md">
			<v-card title="Confirm delete">
				<v-card-text>
					...
					<p>
						Are you sure you want to delete
						<strong>ALL</strong> records? This action is permanent and cannot be undone.
					</p>
				</v-card-text>
				<v-divider></v-divider>
				<v-card-actions>
			...
			<v-card title="Import/Export Data">
				<v-card-text>
					...
					<div v-else-if="formatters.isNullOrEmpty(task.error)">
						<p>
							Export your existing data to
							{{ importExportDescription }} or import a {{ importExportDescription }} file of new values. Any existing values in the
							table with the same name will be updated to match your {{ importExportDescription }} data. Export data first to get a
							template with the correct columns.
						</p>

						<v-alert
							v-if="!formatters.isNullOrEmpty(importExportNotes)"
							type="info"
							icon="$info"
							variant="tonal"
							border="start"
						...
						<select-file-input
							v-model="page.import.form.fileName"
							:value="page.import.form.fileName"
							class="mb-3"
							:label="
								page.import.form.type == 'import_csv'
									? `Select a ${importExportDescription} file to import`
									: `Select where to save your ${importExportDescription} file`
							"
							:fileType="importExportDescription.toLowerCase()"
							required
							:default-file-name="defaultCsvFile"
							:save-dialog="page.import.form.type == 'export_csv'"
		...
		<v-dialog v-model="page.exported.show" :max-width="constants.dialogSizes.md">
			<v-card title="Data Exported">
				<v-card-text>
					<p>
						Your data has been exported to a
						{{ importExportDescription }} file.
					</p>
					<p>
