...
function putDb(formData: any) {
	if (props.isUpdate) return api.put("water_rights/allocation/" + props.item.id, formData, currentProject.getApiHeader());
	else return api.post("water_rights/allocation", formData, currentProject.getApiHeader());
}
...
async function save() {
	...
	if (!valid) {
		...
		try {
			...
			if (props.isUpdate) data.page.saveSuccess = true;
			else
				router.push({
					name: "WaterAllocationEdit",
	...
	if (validateSource()) {
		if (data.sources.obj.obj_typ !== "unl") {
			try {
				...
				data.sources.form.error = errors.logError(
					error,
					"Invalid object name, " +
						data.sources.obj_name +
						". Please make sure the " +
						data.sources.obj.obj_typ +
	...
	if (validateDemand()) {
		if (data.demands.obj.obj_typ === "hru") {
			try {
				...
				data.demands.form.error = errors.logError(
					error,
					"Invalid object name, " +
						data.demands.obj_name +
						". Please make sure the " +
						data.demands.obj.obj_typ +
						" exists in your database.",
				);
			}
		}
		if (data.demands.obj.rcv_obj !== null) {
			try {
				const response = await utilities.getAutoCompleteId(
					data.objTypeToConWaterAllocation[data.demands.obj.rcv_obj],
					data.demands.rcv_obj_name,
				);
	...
	<div>
		...
		<v-form @submit.prevent="save">
			...
			<div v-if="isUpdate">
				...
				<v-alert
					v-if="!data.sources.list || data.sources.list.length < 1"
					type="info"
					icon="$info"
					variant="tonal"
					border="start"
				...
				<v-row>
					<v-col cols="12" md="6">
						<p>
							<strong>Demand.</strong> Demand can be irrigation demand from an hru, municipal demand, or demand to transfer to another
							source object (channel, reservoir, aquifer). For irrigation demand, the hru number, decision table for triggering
							irrigation, and irrigation depth (mm) are input. For municipal demand, a muni number is input. The user then has the
							option to input an average daily demand or a recall name that can be daily, monthly, or annual. For transfer demand, the
							trans number is input and the user can input an average daily demand or a decision table to specify demand.
						</p>
						<p>
							<strong>Treatment.</strong> Treatment is designed for municipal treatment plants or industrial plants that treat or change
							the water chemistry. There are 2 options – 1) recall where you can specify the amount of return flow and 2) delivery where
							you can specify the change in flow and chemistry.
						</p>
					</v-col>
					<v-col cols="12" md="6">
						<p>
							<strong>Receiving.</strong> This can be used to return the treated municipal water (point sources) or to transfer (divert)
							water to other objects. The simple option is the input the return object and number. Or a decision table can be input to
							condition the diversions to multiple objects. Inputting “lost” assumes the water is diverted out of the basin.
						</p>
						<p>
							<strong>Source Allocation.</strong> The sources are listed in order of selection and the fraction from each source is
							input. The other input is compensation – if other sources are not available, the current source may be allowed to
							compensate for the demand. The model goes through all sources in the order listed and allocates based on the fractions.
							The model loops through the sources again, checking to see if compensation is allowed.
						</p>
					</v-col>
				</v-row>

				<v-alert
					v-if="!data.demands.list || data.demands.list.length < 1"
					type="info"
					icon="$info"
					variant="tonal"
					border="start"
			...
			<v-card :title="(data.sources.form.update ? 'Update' : 'Add') + ' Source Object'">
				<v-card-text>
					...
					<v-form :validated="data.sources.form.validated">
						...
						<v-row>
							<v-col cols="12" md="6">
								<div class="form-group">
									<v-select
										label="Object type"
										v-model="data.sources.obj.obj_typ"
										:items="data.sources.obj_typs"
										required
										@update:model-value="data.sources.obj_name = null"
									></v-select>
								</div>
							</v-col>
							<v-col cols="12" md="6">
								<div class="form-group" v-show="data.sources.obj.obj_typ !== 'unl'">
									<auto-complete
										label="Object name"
										v-model="data.sources.obj_name"
										:value="data.sources.obj_name"
										:show-item-link="false"
										:required="data.sources.obj.obj_typ !== 'unl'"
										:table-name="data.objTypeToConWaterAllocation[data.sources.obj.obj_typ]"
										:route-name="data.objTypeRouteWaterAllocation[data.sources.obj.obj_typ].name"
									></auto-complete>
								</div>
							</v-col>
						</v-row>
						...
						<p class="mb-0">Minimum monthly values for object type:</p>
						<ul>
							...
							<v-col>
								<div>
									<v-text-field
										...
										type="number"
										step="any"
										v-model="data.sources.obj.limit_01"
										required
									></v-text-field>
								</div>
							</v-col>
							<v-col>
								<div>
									<v-text-field
										...
										type="number"
										step="any"
										v-model="data.sources.obj.limit_02"
										required
									></v-text-field>
								</div>
							</v-col>
							...
							<v-col>
								<div>
									<v-text-field
										...
										type="number"
										step="any"
										v-model="data.sources.obj.limit_09"
										required
									></v-text-field>
								</div>
							</v-col>
							<v-col>
								<div>
									<v-text-field
										...
										type="number"
										step="any"
										v-model="data.sources.obj.limit_10"
										required
									></v-text-field>
								</div>
							</v-col>
							<v-col>
								<div>
									<v-text-field
										...
										type="number"
										step="any"
										v-model="data.sources.obj.limit_11"
										required
									></v-text-field>
								</div>
							</v-col>
							<v-col>
								<div>
									<v-text-field
										...
										type="number"
										step="any"
										v-model="data.sources.obj.limit_12"
										required
									></v-text-field>
								</div>
							</v-col>
			...
			<v-card title="Confirm delete">
				<v-card-text>
					...
					<p>
						...
						<strong>{{ data.sources.delete.name }}</strong
					...
					<v-row>
						<v-col cols="12" :md="data.demands.isEdit ? 7 : 12">
							<v-form :validated="data.demands.form.validated">
								<div class="form-group">
									<v-text-field
										v-model="data.demands.obj.description"
										label="Name/Description of demand object"
										required
									></v-text-field>
								</div>
								...
								<v-row>
									<v-col cols="12" md="6">
										<div class="form-group">
											<v-select
												label="Object type"
												v-model="data.demands.obj.obj_typ"
												:items="data.demands.obj_typs"
												required
												@update:model-value="data.demands.obj_name = null"
											></v-select>
										</div>
									</v-col>
									<v-col cols="12" md="6">
										<div class="form-group" v-show="data.demands.obj.obj_typ === 'hru'">
											<auto-complete
												label="Object name"
												v-model="data.demands.obj_name"
												:value="data.demands.obj_name"
												:show-item-link="false"
												:required="data.demands.obj.obj_typ === 'hru'"
												:table-name="data.objTypeToConWaterAllocation[data.demands.obj.obj_typ]"
												:route-name="data.objTypeRouteWaterAllocation[data.demands.obj.obj_typ].name"
											></auto-complete>
										</div>
										<div class="form-group" v-show="data.demands.obj.obj_typ === 'hru'">
											<v-text-field
												type="number"
												label="Object number"
												v-model="data.demands.obj.obj_id"
												:required="data.demands.obj.obj_typ !== 'hru'"
											></v-text-field>
										</div>
									</v-col>
								...
								<div class="form-group">
									<v-text-field
										v-model="data.demands.obj.withdr"
										label="Withdrawal type - average day or recall for municipal and divert; irrigation decision table for HRU"
										required
									></v-text-field>
								</div>
								...
								<div class="form-group">
									<v-text-field
										v-model.number="data.demands.obj.amount"
										label="Amount - m3/day for municipal, mm for HRU"
										type="number"
										step="any"
										required
									></v-text-field>
								...
								<div class="form-group">
									<v-select
										label="Water right"
										v-model="data.demands.obj.right"
										:items="data.demands.objRights"
										required
									></v-select>
								</div>
								...
								<div class="form-group">
									<v-select
										label="Object type"
										v-model="data.demands.obj.treat_typ"
										:items="data.demands.objTreatTyps"
										required
									></v-select>
								</div>
								...
								<div class="form-group">
									<v-text-field
										v-model="data.demands.obj.treatment"
										label="Treatment - pointer to the recall or delivery ratio file (optional)"
									></v-text-field>
								</div>
								...
								<v-row>
									<v-col cols="12" md="6">
										<div class="form-group">
											<v-select
												label="Receiving object type"
												v-model="data.demands.obj.rcv_obj"
												:items="data.demands.rcv_objs"
												required
												@update:model-value="data.demands.rcv_obj_name = null"
											></v-select>
										</div>
									</v-col>
									<v-col cols="12" md="6">
										<div class="form-group" v-show="data.demands.obj.rcv_obj !== null">
											<auto-complete
												label="Object name"
												v-model="data.demands.rcv_obj_name"
												:value="data.demands.rcv_obj_name"
												:show-item-link="false"
												:required="data.demands.obj.rcv_obj !== null"
												:table-name="data.objTypeToConWaterAllocation[data.demands.obj.rcv_obj]"
												:route-name="
													data.demands.obj.rcv_obj === null
														? ''
														: data.objTypeRouteWaterAllocation[data.demands.obj.rcv_obj].name
												"
											></auto-complete>
										</div>
									</v-col>
								...
								<div class="form-group">
									<v-text-field
										v-model="data.demands.obj.rcv_dtl"
										label="Receiving object decision table name - to condition water transfers and diversions (optional)"
									></v-text-field>
								</div>

								<v-alert v-if="!data.demands.isEdit" type="info" icon="$info" variant="tonal" border="start" class="mb-4">
									Want to add source objects to this demand object? Click the Save Changes button below first, then from the demand
									table click edit next to the row to add sources.
								</v-alert>
							</v-form>
						</v-col>
						...
						<v-col v-if="data.demands.isEdit" cols="12" md="5">
							<v-alert
								v-if="data.demandSources.list && data.demandSources.list.length < 1"
								type="info"
								icon="$info"
								variant="tonal"
								border="start"
									...
									>
										...
										<template v-slot:item.edit="{ item }">
											<a
												...
												title="Edit"
												aria-label="Edit"
												@click.prevent="editDemandSource(item)"
											>
												<font-awesome-icon :icon="['fas', 'edit']"></font-awesome-icon>
											</a>
										</template>
										<template v-slot:item.delete="{ item }">
											<font-awesome-icon
												...
												class="text-error pointer"
												title="Delete"
												@click="askDemandSourceDelete(item)"
											></font-awesome-icon>
										</template>
										<template v-slot:bottom></template>
									</v-data-table>
		...
		<v-dialog v-model="data.demands.delete.show" :max-width="constants.dialogSizes.md">
			<v-card title="Confirm delete">
				<v-card-text>
					...
					<p>
						Are you sure you want to delete
						<strong>{{ data.demands.delete.name }}</strong
						>? This action is permanent and cannot be undone.
					</p>
				</v-card-text>
				<v-divider></v-divider>
			...
			<v-card :title="(data.demandSources.form.update ? 'Update' : 'Add') + ' Demand Source Object'">
				<v-card-text>
					...
					<v-form :validated="data.demandSources.form.validated">
						...
						<div class="form-group">
							<v-text-field
								v-model.number="data.demandSources.obj.frac"
								label="Fraction allocated from this source"
								type="number"
								step="any"
								required
							></v-text-field>
		...
		<v-dialog v-model="data.demandSources.delete.show" :max-width="constants.dialogSizes.md">
			<v-card title="Confirm delete">
				<v-card-text>
					...
					<p>
						Are you sure you want to delete
						<strong>{{ data.demandSources.delete.name }}</strong
						>? This action is permanent and cannot be undone.
					</p>
				</v-card-text>
				<v-divider></v-divider>
