...
function putDb(data: any) {
	if (props.isUpdate)
		return api.put(
			`regions/ls_units/elements/${props.item.id}`,
			data,
			currentProject.getApiHeader(),
		);
	else
		return api.post(
			`regions/ls_units/elements`,
			data,
			currentProject.getApiHeader(),
		);
}
	...
	if (!valid) {
		...
		try {
			const response = await utilities.getAutoCompleteId(
				constants.objTypeToConTable[props.item.obj_typ],
				props.item.obj_name,
			);
		...
		if (formatters.isNullOrEmpty(page.error)) {
			try {
				const response = await utilities.getAutoCompleteId(
					"ls_unit_def",
					props.item.ls_unit_def_name,
				);
				props.item.ls_unit_def_id = response.data.id;
			} catch (error) {
				page.error = errors.logError(
					error,
					"Invalid landscape unit name, " +
						props.item.ls_unit_def_name +
						". Please make sure it exists in your database.",
				);
			}
		}
		...
		<v-form @submit.prevent="save">
			<v-row>
				...
				<v-col cols="12" md="6">
					<div class="form-group">
						<auto-complete
							label="Assign to Landscape Unit"
							v-model="item.ls_unit_def_name"
							:value="item.ls_unit_def_name"
							:show-item-link="props.isUpdate"
							table-name="ls_unit_def"
							route-name="LandscapeUnitsEdit"
							section="Regions / Landscape Units"
							help-file="ls_unit.def"
							help-db="ls_unit_def"
							api-url="regions/ls_units"
						></auto-complete>
					</div>
				</v-col>
			</v-row>
						...
						<v-select
							...
							:error-messages="
								v$.obj_typ.$errors
									...
									.join(', ')
							"
							@input="v$.obj_typ.$touch"
							@blur="v$.obj_typ.$touch"
						></v-select>
					</div>
				</v-col>
				<v-col cols="12" md="6">
					<div class="form-group">
						<auto-complete
							:label="
								formatters.toUpperFirstLetter(item.obj_typ) +
								' object name'
							"
							v-model="item.obj_name"
							:value="item.obj_name"
							:table-name="
								constants.objTypeToConTable[item.obj_typ]
							"
						></auto-complete>
					</div>
				</v-col>
						...
						<v-text-field
							...
							:error-messages="
								v$.bsn_frac.$errors
									...
									.join(', ')
							"
							@input="v$.bsn_frac.$touch"
							@blur="v$.bsn_frac.$touch"
						></v-text-field>
					</div>
				</v-col>
				<v-col cols="12" md="4">
						...
						<v-text-field
							...
							:error-messages="
								v$.sub_frac.$errors
									...
									.join(', ')
							"
							@input="v$.sub_frac.$touch"
							@blur="v$.sub_frac.$touch"
						></v-text-field>
					</div>
				</v-col>
				<v-col cols="12" md="4">
						...
						<v-text-field
							...
							:error-messages="
								v$.reg_frac.$errors
									...
									.join(', ')
							"
							@input="v$.reg_frac.$touch"
							@blur="v$.reg_frac.$touch"
						></v-text-field>
					</div>
				</v-col>
			</v-row>
