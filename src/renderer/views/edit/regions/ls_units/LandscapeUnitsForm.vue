...
function putDb(data: any) {
	if (props.isUpdate)
		return api.put(
			`regions/ls_units/${props.item.id}`,
			data,
	...
	<div>
		...
		<v-form @submit.prevent="save">
			...
			<div v-if="isUpdate">
				...
				<div
					v-if="!item.elements || item.elements.length < 1"
				...
				<div v-else>
					<grid-view
						:api-url="`${table.apiUrl}/${props.item.id}`"
						:headers="table.headers"
						:edit-path-prefix="table.editPathPrefix"
					></grid-view>
				</div>
			</div>
			...
			<action-bar>
				...
				<v-btn
					...
					class="mr-2"
					v-if="isUpdate"
					to="/edit/regions/ls_units/elements/create/"
