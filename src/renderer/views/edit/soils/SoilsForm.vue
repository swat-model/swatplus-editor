...
function putDb(formData: any) {
	if (props.isUpdate) return api.put("soils/items/" + props.item.id, formData, currentProject.getApiHeader());
	else return api.post("soils/items", formData, currentProject.getApiHeader());
}
		...
		<v-form @submit.prevent="save">
			...
			<div class="form-group">
				<v-text-field
					...
					@input="v$.texture.$touch"
					@blur="v$.texture.$touch"
				></v-text-field>
			</div>
			...
			<v-row>
				...
				<v-col cols="12" md="6">
					<div class="form-group mb-0">
						<v-text-field
							v-model.number.trim="item.dp_tot"
							label="Maximum rooting depth of soil profile (mm)"
							type="number"
							step="any"
							:error-messages="v$.dp_tot.$errors.map((e) => e.$message).join(', ')"
							@input="v$.dp_tot.$touch"
							@blur="v$.dp_tot.$touch"
						></v-text-field>
					</div>
				</v-col>
			...
			<v-row>
				<v-col cols="12" md="6">
					<div class="form-group mb-0">
						<v-text-field
							v-model.number.trim="item.anion_excl"
							label="Fraction of porosity (void space) from which anions are excluded"
							type="number"
							step="any"
							:error-messages="v$.anion_excl.$errors.map((e) => e.$message).join(', ')"
							@input="v$.anion_excl.$touch"
							@blur="v$.anion_excl.$touch"
						></v-text-field>
					</div>
				</v-col>
				<v-col cols="12" md="6">
					<div class="form-group mb-0">
						<v-text-field
							v-model.number.trim="item.perc_crk"
							label="Potential or maximum crack volume of the soil profile expressed as a fraction of the total soil volume"
							type="number"
							step="any"
							:error-messages="v$.perc_crk.$errors.map((e) => e.$message).join(', ')"
							@input="v$.perc_crk.$touch"
							@blur="v$.perc_crk.$touch"
						></v-text-field>
					</div>
				</v-col>
			</v-row>
				...
				<div v-if="data.layers.list && data.layers.list.length > 0">
					<v-card>
						<v-data-table
							class="data-table"
							density="compact"
							:items="data.layers.list"
							:items-per-page="-1"
							:headers="data.layers.fields"
						>
							<template v-slot:item.edit="{ item }">
								<a href="#" class="text-decoration-none text-primary" title="Edit" aria-label="Edit" @click.prevent="edit(item)">
				...
				<v-card-text>
					...
					<v-form :validated="data.layers.form.validated">
						<v-table class="table-editor" density="compact">
							...
							<tbody>
								<tr-var-editor
									v-for="(v, i) in props.vars"
									:key="i"
									:id="'item_' + v.name"
									required
									show-range
									v-model="data.layers.obj[v.name]"
									:value="data.layers.obj[v.name]"
									:var-def="v"
								></tr-var-editor>
							</tbody>
						</v-table>
			...
			<v-card title="Confirm delete">
				<v-card-text>
					...
					<p>
						...
						<strong>{{ data.layers.delete.name }}</strong
