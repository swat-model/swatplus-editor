...
function exportStationsTemplate() {
	...
	let files = electron.saveFileDialog({
		filters: filters,
		defaultPath: "atmo-weather-station-map.csv",
	});
...
<template>
	<project-container :loading="page.loading" :load-error="page.error">
		<div v-if="route.name === 'StationsAtmo'">
			...
			<v-alert
				v-if="!page.data.has_weather_stations"
				type="info"
				icon="$info"
				variant="tonal"
				border="start"
			...
			<div v-else>
				<p>
					Configure atmospheric deposition data below. The simplest
					method is to click the <strong>Import Data</strong> button
					at the bottom of the screen. Otherwise use the form below,
					making sure each station data and timestep created match the
					selected settings. If using monthly or yearly data, be sure
					the dates fall within your model simulation dates.
				</p>
					...
					<v-row>
						<v-col cols="12" md="3">
							<div class="form-group mb-0">
								<v-select
									label="Timestep"
									v-model="page.data.timestep"
									@update:model-value="changeTimestep"
									...
									:error-messages="
										vd$.timestep.$errors
											...
											.join(', ')
									"
									@input="vd$.timestep.$touch"
									@blur="vd$.timestep.$touch"
								></v-select>
							</div>
						</v-col>
						<v-col
							cols="12"
							md="3"
							v-if="page.data.timestep === 'mo'"
						>
							<div class="form-group mb-0">
								<v-text-field
									v-model.number="page.data.mo_init"
									label="Start Month"
									type="number"
									:error-messages="
										vd$.mo_init.$errors
											...
											.join(', ')
									"
									@input="vd$.mo_init.$touch"
									@blur="vd$.mo_init.$touch"
								></v-text-field>
							</div>
						</v-col>
						<v-col
							cols="12"
							md="3"
							v-if="page.data.timestep !== 'aa'"
						>
							<div class="form-group mb-0">
								<v-text-field
									v-model.number="page.data.yr_init"
									label="Start Year"
									type="number"
									:error-messages="
										vd$.yr_init.$errors
											...
											.join(', ')
									"
									@input="vd$.yr_init.$touch"
									@blur="vd$.yr_init.$touch"
								></v-text-field>
							</div>
						</v-col>
						<v-col
						...
						>
							<div class="form-group mb-0">
								<v-text-field
									v-model.number="page.data.num_aa"
									:label="`Number of ${page.data.timestep === 'mo' ? 'Months' : 'Years'}`"
									type="number"
									:error-messages="
										vd$.num_aa.$errors
											.map((e) => e.$message)
											.join(', ')
									"
									@input="vd$.num_aa.$touch"
									@blur="vd$.num_aa.$touch"
								></v-text-field>
							</div>
						</v-col>
					</v-row>
				...
				>
					<v-card title="Confirm delete">
						<v-card-text>
							<error-alert
								:text="page.delete.error"
							></error-alert>

							<p>
								Are you sure you want to delete
								<strong>ALL</strong> atmospheric deposition
								stations? This action is permanent and cannot be
								undone.
							</p>
						</v-card-text>
						<v-divider></v-divider>
						<v-card-actions>
							<v-btn
								...
								>Delete All</v-btn
							>
							<v-btn @click="page.delete.show = false"
								>Cancel</v-btn
							>
						</v-card-actions>
					</v-card>
				...
				>
					<v-card title="Import Atmospheric Deposition Data">
						<v-card-text>
							<error-alert
								:text="page.import.error"
							></error-alert>
							<stack-trace-error
								v-if="!formatters.isNullOrEmpty(task.error)"
								error-title="There was an error importing your data."
							...
							<div
								v-else-if="formatters.isNullOrEmpty(task.error)"
							>
								<p>
									Any existing atmospheric deposition data
									will be deleted.
								</p>
								...
								<div class="form-group mb-0">
									<v-select
										...
										v-model="page.import.form.importMethod"
										:items="
											page.import.options.importMethod
										"
										:error-messages="
											v$.importMethod.$errors
												.map((e) => e.$message)
												.join(', ')
										"
										@input="v$.importMethod.$touch"
										@blur="v$.importMethod.$touch"
									></v-select>
								</div>

								<div class="form-group mb-0 d-flex">
									<select-file-input
										v-model="page.import.form.file1"
										:value="page.import.form.file1"
										label="Atmospheric deposition file"
										:fileType="
											page.import.form.importMethod
										"
										required
										invalidFeedback="Please select a file"
										class="flex-fill mr-5"
									></select-file-input>
										...
										<v-list>
											<v-list-item
												><open-in-browser
													...
													:url="`https://plus.swat.tamu.edu/downloads/sample_files/atmo/atmo_avgannual.${selectedFileExt}`"
													text="Average Annual"
												></open-in-browser
											></v-list-item>
											<v-list-item
												><open-in-browser
													class="text-decoration-none text-high-emphasis"
													:url="`https://plus.swat.tamu.edu/downloads/sample_files/atmo/atmo_yearly.${selectedFileExt}`"
													text="Yearly"
												></open-in-browser
											></v-list-item>
											<v-list-item
												><open-in-browser
													class="text-decoration-none text-high-emphasis"
													:url="`https://plus.swat.tamu.edu/downloads/sample_files/atmo/atmo_monthly.${selectedFileExt}`"
													text="Monthly"
												></open-in-browser
											></v-list-item>
										</v-list>
								...
								<p>
									Atmospheric deposition stations need to be
									mapped to your existing weather stations.
									Click the Export Template button to get a
									CSV containing your weather stations. Fill
									the first column of the CSV with your
									atmospheric deposition station names.
								</p>

								<div class="form-group mb-0 d-flex">
									<select-file-input
										v-model="page.import.form.file2"
										:value="page.import.form.file2"
									...
									<v-btn
										...
										variant="flat"
										class="ml-auto mt-2"
										@click="exportStationsTemplate"
										>Export Template File</v-btn
									>
								</div>
							</div>
						</v-card-text>
						...
						<v-card-actions>
							<v-btn
								v-if="formatters.isNullOrEmpty(task.error)"
								:loading="task.running || page.import.saving"
								@click="importData"
								color="primary"
								variant="text"
								>Import Data</v-btn
				...
				>
					<v-card title="Data Exported">
						<v-card-text>
							...
							<p>
								<open-file
									:file-path="page.exported.fileName"
									text="Open file"
									button
									color="primary"
								></open-file>
						...
						<v-divider></v-divider>
						<v-card-actions>
							<v-btn @click="page.exported.show = false"
								>Close</v-btn
							>
						</v-card-actions>
					</v-card>
