<script setup lang="ts">
	...
	async function get() {
		...
		try {
			const response = await api.get(`climate/wgn/${route.params.id}`, currentProject.getApiHeader());
			errors.log(response.data);
			data.item = response.data;
		} catch (error) {
			data.page.error = errors.logError(error, 'Unable to get weather generator from database.');
		}
...
<template>
	<project-container :loading="data.page.loading" :load-error="data.page.error">
		...
		<wgn-form :item="data.item" is-update></wgn-form>
	</project-container>
</template>