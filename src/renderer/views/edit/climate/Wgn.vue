...
let page: any = reactive({
	...
	import: {
		...
		form: {
			method: "database",
			csvFile1: null as string | null,
			csvFile2: null as string | null,
		...
		options: {
			methods: [
				...
				{
					title: "Two CSV files (separate file for stations and monthly values)",
					value: "two_file",
				},
...
const formRules = computed(() => ({
	...
	csvFile2: {
		required: helpers.withMessage(
			"Value is required",
			requiredIf(() => {
				return page.import.form.method === "two_file";
			}),
		),
	},
	db: {
		required: helpers.withMessage(
			"Value is required",
			requiredIf(() => {
				return page.import.form.method === "database";
			}),
		),
	},
	table: {
...
<template>
	<project-container :loading="page.loading" :load-error="page.error">
		<div v-if="route.name === 'Wgn'">
			...
			<v-alert
				v-if="!page.validate.loading && page.validate.is_invalid"
				type="warning"
				icon="$warning"
				variant="tonal"
				border="start"
						...
						<div v-else-if="formatters.isNullOrEmpty(task.error)">
							...
							<div v-if="page.import.form.method === 'database'">
								...
								>
									Need wgn data?
									<open-in-browser
										url="https://plus.swat.tamu.edu/downloads/swatplus_wgn.zip"
										text="Download the global SWAT+ wgn database (180MB)."
									></open-in-browser>
								</v-alert>

								<div class="form-group mb-0">
									<select-file-input
										v-model="page.import.form.db"
							...
							>
								...
								<div class="form-group mb-0">
									<select-file-input
										v-model="page.import.form.csvFile1"
								...
								<div class="form-group mb-0">
									<select-file-input
										v-model="page.import.form.csvFile2"
