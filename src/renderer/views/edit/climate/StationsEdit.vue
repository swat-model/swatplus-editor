<script setup lang="ts">
	...
	async function get() {
		...
		try {
			const response = await api.get(`climate/stations/${route.params.id}`, currentProject.getApiHeader());
			errors.log(response.data);
			data.item = response.data;
		} catch (error) {
			data.page.error = errors.logError(error, 'Unable to get weather station from database.');
		}
...
<template>
	<project-container :loading="data.page.loading" :load-error="data.page.error">
		...
		<stations-form is-update :item="data.item" />
	</project-container>
</template>
