...
const { api, constants, currentProject, errors, formatters, utilities } =
	useHelpers();
...
let page: any = reactive({
	...
	saveSuccess: false,
	values: {
		list: (props.item.monthly_values || []) as any[],
		obj: {
...
function putDb(data: any) {
	if (props.isUpdate)
		return api.put(
			`climate/wgn/${props.item.id}`,
			data,
			currentProject.getApiHeader(),
		);
	else return api.post(`climate/wgn`, data, currentProject.getApiHeader());
}
...
const monthOptions = computed(() => {
	if (page.values.list.length < 1) return constants.monthSelectList;
		...
		let usedMonths = [] as number[];
		for (let v of page.values.list) {
			usedMonths.push(v.month);
		}

		return constants.monthSelectList.filter(function (el) {
			return !usedMonths.includes(el.value);
		});
	}
});
...
async function getValues() {
	...
	try {
		...
		page.error = errors.logError(
			error,
			"Unable to get wgn monthly values from database.",
		);
	}
...
async function saveValues() {
	...
	if (!valid) {
		...
		if (page.values.form.update) {
			action = api.put(
				`climate/wgn/mon-value/${page.values.form.id}`,
				page.values.obj,
				currentProject.getApiHeader(),
			);
		} else {
			action = api.post(
				`climate/wgn/mon-table/${props.item.id}`,
				page.values.obj,
				currentProject.getApiHeader(),
			);
		}
		...
		try {
			...
			page.values.form.error = errors.logError(
				error,
				"Unable to save values to database.",
			);
		}
	}
...
async function confirmDelete() {
	...
	try {
		await api.delete(
			`climate/wgn/mon-value/${page.values.delete.id}`,
			currentProject.getApiHeader(),
		);
		await getValues();
		page.values.delete.show = false;
	} catch (error) {
		page.values.delete.error = errors.logError(
			error,
...
<template>
	<div>
		...
		<success-alert
			v-model="page.saveSuccess"
			:show="page.saveSuccess"
		></success-alert>
		...
		<v-form @submit.prevent="save">
				...
				<v-col cols="12" md="6">
					<div class="form-group mb-0">
						<v-text-field
							...
							step="any"
							:error-messages="
								v$.lat.$errors.map((e) => e.$message).join(', ')
							"
							@input="v$.lat.$touch"
							@blur="v$.lat.$touch"
						></v-text-field>
				...
				<v-col cols="12" md="6">
					<div class="form-group mb-0">
						<v-text-field
							...
							step="any"
							:error-messages="
								v$.lon.$errors.map((e) => e.$message).join(', ')
							"
							@input="v$.lon.$touch"
							@blur="v$.lon.$touch"
						></v-text-field>
			...
			<v-row>
				...
				<v-col cols="12" md="6">
					<div class="form-group mb-0">
						<v-text-field
							v-model="item.rain_yrs"
							label="Years of recorded max monthly 0.5h rainfall data"
							type="number"
							step="any"
							:error-messages="
								v$.rain_yrs.$errors
									...
									.join(', ')
							"
							@input="v$.rain_yrs.$touch"
							@blur="v$.rain_yrs.$touch"
						></v-text-field>
					</div>
				</v-col>
			</v-row>
			...
			<div v-if="props.isUpdate">
				...
				<div
					v-if="!page.values.list || page.values.list.length < 1"
						...
						>
							<thead>
								<tr class="bg-surface">
									...
									<th
										class="bg-secondary-tonal text-right"
										v-for="f in page.values.fields"
										:key="f.key"
									>
										{{
											formatters.isNullOrEmpty(f.label)
												? f.key
												: f.label
										}}
									</th>
							...
							<tbody v-if="page.values.loading">
								<tr
									v-for="(v, i) in Array.from(Array(12))"
									:key="i"
								>
									<td class="min"></td>
									<td
							...
							<tbody v-else>
								<tr
									v-for="(row, j) in page.values.list"
									:key="j"
								>
									<td class="min">
										<a
									...
									<td class="min">
										<font-awesome-icon
											...
											title="Delete"
											@click="askDelete(row.id, row.name)"
										></font-awesome-icon>
									</td>
								</tr>
							</tbody>
			...
			<action-bar>
				...
				<v-btn
					...
					v-if="
						isUpdate &&
						(!page.values.list || page.values.list.length < 12)
					"
					@click="add"
					>Add Monthly Values</v-btn
				>
		...
		<v-dialog
			v-model="page.values.form.show"
			:max-width="constants.dialogSizes.lg"
		>
			<v-card
				:title="
					page.values.form.update
			...
			>
				<v-card-text>
					...
					<div class="form-group mb-0">
						<v-select
							label="Month"
							v-model="page.values.obj.month"
							:items="
								page.values.form.update
									? constants.monthSelectList
									: monthOptions
							"
					...
					<v-row>
						<v-col cols="12" md="6">
							<div class="form-group mb-0">
								<v-text-field
									v-model="page.values.obj.tmp_max_ave"
									label="Avg max daily temperature (°C)"
									type="number"
									step="any"
									:error-messages="
										vm$.tmp_max_ave.$errors
											.map((e) => e.$message)
											.join(', ')
									"
									@input="vm$.tmp_max_ave.$touch"
									@blur="vm$.tmp_max_ave.$touch"
								></v-text-field>
							</div>
						</v-col>
						<v-col cols="12" md="6">
							<div class="form-group mb-0">
								<v-text-field
									v-model="page.values.obj.tmp_min_ave"
									label="Avg min daily temperature (°C)"
									type="number"
									step="any"
									:error-messages="
										vm$.tmp_min_ave.$errors
											.map((e) => e.$message)
											.join(', ')
									"
									@input="vm$.tmp_min_ave.$touch"
									@blur="vm$.tmp_min_ave.$touch"
								></v-text-field>
							</div>
						</v-col>
					</v-row>
					...
					<v-row>
						<v-col cols="12" md="6">
							<div class="form-group mb-0">
								<v-text-field
									v-model="page.values.obj.tmp_max_sd"
									label="Std deviation for daily max temperature (°C)"
									type="number"
									step="any"
									:error-messages="
										vm$.tmp_max_sd.$errors
											.map((e) => e.$message)
											.join(', ')
									"
									@input="vm$.tmp_max_sd.$touch"
									@blur="vm$.tmp_max_sd.$touch"
								></v-text-field>
							</div>
						</v-col>
						<v-col cols="12" md="6">
							<div class="form-group mb-0">
								<v-text-field
									v-model="page.values.obj.tmp_min_sd"
									label="Std deviation for daily min temperature (°C)"
									type="number"
									step="any"
									:error-messages="
										vm$.tmp_min_sd.$errors
											.map((e) => e.$message)
											.join(', ')
									"
									@input="vm$.tmp_min_sd.$touch"
									@blur="vm$.tmp_min_sd.$touch"
								></v-text-field>
							</div>
						</v-col>
					</v-row>
					...
					<v-row>
						<v-col cols="12" md="6">
							<div class="form-group mb-0">
								<v-text-field
									v-model="page.values.obj.pcp_ave"
									label="Avg total monthly precipitation (mm)"
									type="number"
									step="any"
									:error-messages="
										vm$.pcp_ave.$errors
											...
											.join(', ')
									"
									@input="vm$.pcp_ave.$touch"
									@blur="vm$.pcp_ave.$touch"
								></v-text-field>
							</div>
						</v-col>
						<v-col cols="12" md="6">
							<div class="form-group mb-0">
								<v-text-field
									v-model="page.values.obj.pcp_sd"
									label="Std deviation for avg daily precipitation (mm/day)"
									type="number"
									step="any"
									:error-messages="
										vm$.pcp_sd.$errors
											...
											.join(', ')
									"
									@input="vm$.pcp_sd.$touch"
									@blur="vm$.pcp_sd.$touch"
								></v-text-field>
							</div>
						</v-col>
					</v-row>
					...
					<v-row>
						<v-col cols="12" md="6">
							<div class="form-group mb-0">
								<v-text-field
									v-model="page.values.obj.pcp_skew"
									label="Skew coefficient for avg daily precipitation (mm)"
									type="number"
									step="any"
									:error-messages="
										vm$.pcp_skew.$errors
											...
											.join(', ')
									"
									@input="vm$.pcp_skew.$touch"
									@blur="vm$.pcp_skew.$touch"
								></v-text-field>
							</div>
						</v-col>
						<v-col cols="12" md="6">
							<div class="form-group mb-0">
								<v-text-field
									v-model="page.values.obj.wet_dry"
									label="Probability of a wet day after a dry day"
									type="number"
									step="any"
									:error-messages="
										vm$.wet_dry.$errors
											...
											.join(', ')
									"
									@input="vm$.wet_dry.$touch"
									@blur="vm$.wet_dry.$touch"
								></v-text-field>
							</div>
						</v-col>
					</v-row>
					...
					<v-row>
						<v-col cols="12" md="6">
							<div class="form-group mb-0">
								<v-text-field
									v-model="page.values.obj.wet_wet"
									label="Probability of a wet day after a wet day"
									type="number"
									step="any"
									:error-messages="
										vm$.wet_wet.$errors
											...
											.join(', ')
									"
									@input="vm$.wet_wet.$touch"
									@blur="vm$.wet_wet.$touch"
								></v-text-field>
							</div>
						</v-col>
						<v-col cols="12" md="6">
							<div class="form-group mb-0">
								<v-text-field
									v-model="page.values.obj.pcp_days"
									label="Avg # of precipitation days in month"
									type="number"
									step="any"
									:error-messages="
										vm$.pcp_days.$errors
											...
											.join(', ')
									"
									@input="vm$.pcp_days.$touch"
									@blur="vm$.pcp_days.$touch"
								></v-text-field>
							</div>
						</v-col>
					</v-row>
					...
					<v-row>
						<v-col cols="12" md="6">
							<div class="form-group mb-0">
								<v-text-field
									v-model="page.values.obj.pcp_hhr"
									label="Max 0.5hr rainfall in entire period (mm)"
									type="number"
									step="any"
									:error-messages="
										vm$.pcp_hhr.$errors
											...
											.join(', ')
									"
									@input="vm$.pcp_hhr.$touch"
									@blur="vm$.pcp_hhr.$touch"
								></v-text-field>
							</div>
						</v-col>
						<v-col cols="12" md="6">
							<div class="form-group mb-0">
								<v-text-field
									v-model="page.values.obj.slr_ave"
									label="Avg daily solar radiation (MJ/m2/day)"
									type="number"
									step="any"
									:error-messages="
										vm$.slr_ave.$errors
											...
											.join(', ')
									"
									@input="vm$.slr_ave.$touch"
									@blur="vm$.slr_ave.$touch"
								></v-text-field>
							</div>
						</v-col>
					</v-row>
					...
					<v-row>
						<v-col cols="12" md="6">
							<div class="form-group mb-0">
								<v-text-field
									v-model="page.values.obj.dew_ave"
									label="Avg daily dew point temperature (°C)"
									type="number"
									step="any"
									:error-messages="
										vm$.dew_ave.$errors
											...
											.join(', ')
									"
									@input="vm$.dew_ave.$touch"
									@blur="vm$.dew_ave.$touch"
								></v-text-field>
							</div>
						</v-col>
						<v-col cols="12" md="6">
							<div class="form-group mb-0">
								<v-text-field
									v-model="page.values.obj.wnd_ave"
									...
									:error-messages="
										vm$.wnd_ave.$errors
											...
											.join(', ')
									"
									@input="vm$.wnd_ave.$touch"
									@blur="vm$.wnd_ave.$touch"
								></v-text-field>
							</div>
						</v-col>
					</v-row>
				...
				<v-card-actions>
					<v-btn
						@click="saveValues"
						:loading="page.values.form.saving"
						color="primary"
						variant="text"
						>Save Changes</v-btn
					>
		...
		<v-dialog
			v-model="page.values.delete.show"
			...
			<v-card title="Confirm delete">
				<v-card-text>
					...
					<p>
						...
						<strong>{{ page.values.delete.name }}</strong
				...
				<v-card-actions>
					<v-btn
						@click="confirmDelete"
						:loading="page.values.delete.saving"
						color="error"
						variant="text"
						>Delete</v-btn
					>
					<v-btn @click="page.values.delete.show = false"
						>Cancel</v-btn
					>
				</v-card-actions>
			</v-card>
