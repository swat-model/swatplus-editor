...
function putDb(data: any) {
	if (props.isUpdate)
		return api.put(
			`climate/atmo/stations/${props.item.id}`,
			data,
			currentProject.getApiHeader(),
		);
	else
		return api.post(
			`climate/atmo/stations`,
			data,
			currentProject.getApiHeader(),
		);
}
...
async function getValues() {
	...
	try {
		...
		page.error = errors.logError(
			error,
			"Unable to get atmo. station values from database.",
		);
	}
...
async function saveValues() {
	...
	if (!valid) {
		...
		if (page.values.form.update) {
			action = api.put(
				`climate/atmo/values/${page.values.form.id}`,
				page.values.obj,
				currentProject.getApiHeader(),
			);
		} else {
...
async function confirmDelete() {
	...
	try {
		await api.delete(
			`climate/atmo/values/${page.values.delete.id}`,
			currentProject.getApiHeader(),
		);
		await getValues();
...
<template>
	<div>
		...
		>
			<v-card
				:title="
					page.values.form.update ? 'Update values' : 'Add values'
				"
			>
				<v-card-text>
					...
					<div class="form-group">
						<v-text-field
							v-model="page.values.obj.timestep"
							label="Timestep"
							type="number"
							:error-messages="
								vo$.timestep.$errors
									...
									.join(', ')
							"
							@input="vo$.timestep.$touch"
							@blur="vo$.timestep.$touch"
					...
					<div class="form-group mb-0">
						<v-text-field
							v-model="page.values.obj.nh4_wet"
							label="Wet deposition of ammonia nitrogen (NH4)"
							type="number"
							step="any"
							suffix="mg/l"
							:error-messages="
								vo$.nh4_wet.$errors
									...
									.join(', ')
							"
							@input="vo$.nh4_wet.$touch"
							@blur="vo$.nh4_wet.$touch"
						></v-text-field>
					</div>

					<div class="form-group mb-0">
						<v-text-field
							v-model="page.values.obj.no3_wet"
							label="Wet deposition of nitrate nitrogen (NO3)"
							type="number"
							step="any"
							suffix="mg/l"
							:error-messages="
								vo$.no3_wet.$errors
									...
									.join(', ')
							"
							@input="vo$.no3_wet.$touch"
							@blur="vo$.no3_wet.$touch"
						></v-text-field>
					</div>

					<div class="form-group mb-0">
						<v-text-field
							v-model="page.values.obj.nh4_dry"
							label="Dry deposition of ammonia nitrogen (NH4)"
							type="number"
							step="any"
							suffix="kg/ha/yr"
							:error-messages="
								vo$.nh4_dry.$errors
									...
									.join(', ')
							"
							@input="vo$.nh4_dry.$touch"
							@blur="vo$.nh4_dry.$touch"
						></v-text-field>
					</div>

					<div class="form-group mb-0">
						<v-text-field
							v-model="page.values.obj.no3_dry"
							label="Dry deposition of nitrate nitrogen (NO3)"
							type="number"
							step="any"
							suffix="kg/ha/yr"
							:error-messages="
								vo$.no3_dry.$errors
									...
									.join(', ')
							"
							@input="vo$.no3_dry.$touch"
							@blur="vo$.no3_dry.$touch"
						></v-text-field>
					</div>
				</v-card-text>
				<v-divider></v-divider>
