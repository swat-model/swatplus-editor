...
async function saveValues() {
	...
	if (!valid) {
		data.page.values.form.error =
			"Please fix the errors below and try again.";
	} else {
		let action;
		if (data.page.values.form.update) {
			action = api.put(
				`salts/atmo-values/${data.page.values.form.id}`,
				data.page.values.obj,
				currentProject.getApiHeader(),
			);
		} else {
			action = api.post(
				`salts/atmo/${route.params.id}`,
				data.page.values.obj,
				currentProject.getApiHeader(),
			);
		}
		...
		try {
			...
			data.page.values.form.error = errors.logError(
				error,
...
async function confirmDelete() {
	...
	try {
		await api.delete(
			`salts/atmo-values/${data.page.values.delete.id}`,
			currentProject.getApiHeader(),
		);
		await getValues();
		data.page.values.delete.show = false;
	} catch (error) {
		data.page.values.delete.error = errors.logError(
			error,
...
<template>
	...
	>
		<div v-if="route.name == 'ConstituentsSaltsAtmoEdit'">
			<file-header
				input-file="constituents.cs"
				docs-path="constituents"
				use-io
			>
				<router-link to="/edit/constituents/salts"
					>Salt Constituents</router-link
				>
				/
				<router-link to="/edit/constituents/salts/atmo"
					>Atmospheric Deposition</router-link
				>
				/ Edit
			</file-header>
			...
			<div class="form-group">
				<v-text-field
					...
					label="Name"
					readonly
					hint="Name cannot be modified. Matches one of your existing atmospheric deposition stations."
				></v-text-field>
			</div>

			<h2 class="text-h5 my-3">Values</h2>
			<v-alert
				v-if="!data.item.values || data.item.values.length < 1"
				variant="tonal"
				type="info"
			>
				This station does not have any values.
				<a href="#" @click.prevent="add" class="text-primary"
					>Add now.</a
				>
			</v-alert>
				...
				<v-card>
					<v-table class="data-table" fixed-header density="compact">
						...
						<tbody v-if="data.page.values.loading">
							...
							>
								<td class="min"></td>
								<td v-for="f in data.fields" :key="f.key">
									<v-skeleton-loader
										type="text"
										max-width="150"
									></v-skeleton-loader>
						...
						<tbody v-else>
							<tr v-for="(row, j) in data.item.values" :key="j">
								...
								<td
									...
									class="text-right"
								>
									<span v-if="f.key === 'timestep'">{{
										row[f.key]
									}}</span>
									<span v-else>{{
										formatters.toNumberFormat(row[f.key], 3)
									}}</span>
								</td>
								<td class="min">
									<font-awesome-icon
			...
			<v-dialog
				v-model="data.page.values.form.show"
				:max-width="constants.dialogSizes.lg"
			>
				<v-card
					:title="
						data.page.values.form.update
							? 'Update values'
							: 'Add values'
					"
				>
					<v-card-text>
						<error-alert
							:text="data.page.values.form.error"
						></error-alert>

						<div class="form-group">
							<v-text-field
								v-model="data.page.values.obj.timestep"
								label="Timestep"
								type="number"
								:error-messages="
									v$.timestep.$errors
										...
										.join(', ')
								"
								@input="v$.timestep.$touch"
								@blur="v$.timestep.$touch"
								hint="Average Annual = 0; Yearly = YYYY; Monthly = YYYYMM"
								persistent-hint
							></v-text-field>
						</div>
						...
						<v-row>
							<v-col cols="12" md="6">
								<div class="form-group mb-0">
									<v-text-field
										v-model="data.page.values.obj.so4_wet"
										label="Wet deposition of sulfate (so4)"
										type="number"
										step="any"
										suffix="g/m3"
										:error-messages="
											v$.so4_wet.$errors
												...
												.join(', ')
										"
										@input="v$.so4_wet.$touch"
										@blur="v$.so4_wet.$touch"
									></v-text-field>
								</div>
								<div class="form-group mb-0">
									<v-text-field
										v-model="data.page.values.obj.ca_wet"
										label="Wet deposition of calcium (ca)"
										type="number"
										step="any"
										suffix="g/m3"
										:error-messages="
											v$.ca_wet.$errors
												...
												.join(', ')
										"
										@input="v$.ca_wet.$touch"
										@blur="v$.ca_wet.$touch"
									></v-text-field>
								</div>
								<div class="form-group mb-0">
									<v-text-field
										v-model="data.page.values.obj.mg_wet"
										label="Wet deposition of magnesium (mg)"
										type="number"
										step="any"
										suffix="g/m3"
										:error-messages="
											v$.mg_wet.$errors
												...
												.join(', ')
										"
										@input="v$.mg_wet.$touch"
										@blur="v$.mg_wet.$touch"
									></v-text-field>
								</div>
								<div class="form-group mb-0">
									<v-text-field
										v-model="data.page.values.obj.na_wet"
										label="Wet deposition of sodium (na)"
										type="number"
										step="any"
										suffix="g/m3"
										:error-messages="
											v$.na_wet.$errors
												...
												.join(', ')
										"
										@input="v$.na_wet.$touch"
										@blur="v$.na_wet.$touch"
									></v-text-field>
								</div>
								<div class="form-group mb-0">
									<v-text-field
										v-model="data.page.values.obj.k_wet"
										label="Wet deposition of potassium (k)"
										type="number"
										step="any"
										suffix="g/m3"
										:error-messages="
								...
								<div class="form-group mb-0">
									<v-text-field
										v-model="data.page.values.obj.cl_wet"
										label="Wet deposition of chlorine (cl)"
										type="number"
										step="any"
										suffix="g/m3"
										:error-messages="
											v$.cl_wet.$errors
												...
												.join(', ')
										"
										@input="v$.cl_wet.$touch"
										@blur="v$.cl_wet.$touch"
									></v-text-field>
								</div>
								<div class="form-group mb-0">
									<v-text-field
										v-model="data.page.values.obj.co3_wet"
										label="Wet deposition of carbonate (co3)"
										type="number"
										step="any"
										suffix="g/m3"
										:error-messages="
											v$.co3_wet.$errors
												...
												.join(', ')
										"
										@input="v$.co3_wet.$touch"
										@blur="v$.co3_wet.$touch"
									></v-text-field>
								</div>
								<div class="form-group mb-0">
									<v-text-field
										v-model="data.page.values.obj.hco3_wet"
										label="Wet deposition of hydrogen carbonate (hco3)"
										type="number"
										step="any"
										suffix="g/m3"
										:error-messages="
											v$.hco3_wet.$errors
												...
												.join(', ')
										"
										@input="v$.hco3_wet.$touch"
										@blur="v$.hco3_wet.$touch"
									></v-text-field>
								</div>
							</v-col>
							<v-col cols="12" md="6">
								<div class="form-group mb-0">
									<v-text-field
										v-model="data.page.values.obj.so4_dry"
										label="Dry deposition of sulfate (so4)"
										type="number"
										step="any"
										suffix="kg/ha"
										:error-messages="
											v$.so4_dry.$errors
												...
												.join(', ')
										"
										@input="v$.so4_dry.$touch"
										@blur="v$.so4_dry.$touch"
									></v-text-field>
								</div>
								<div class="form-group mb-0">
									<v-text-field
										v-model="data.page.values.obj.ca_dry"
										label="Dry deposition of calcium (ca)"
										type="number"
										step="any"
										suffix="kg/ha"
										:error-messages="
											v$.ca_dry.$errors
												...
												.join(', ')
										"
										@input="v$.ca_dry.$touch"
										@blur="v$.ca_dry.$touch"
									></v-text-field>
								</div>
								<div class="form-group mb-0">
									<v-text-field
										v-model="data.page.values.obj.mg_dry"
										label="Dry deposition of magnesium (mg)"
										type="number"
										step="any"
										suffix="kg/ha"
										:error-messages="
											v$.mg_dry.$errors
												...
												.join(', ')
										"
										@input="v$.mg_dry.$touch"
										@blur="v$.mg_dry.$touch"
									></v-text-field>
								</div>
								<div class="form-group mb-0">
									<v-text-field
										v-model="data.page.values.obj.na_dry"
										label="Dry deposition of sodium (na)"
										type="number"
										step="any"
										suffix="kg/ha"
										:error-messages="
											v$.na_dry.$errors
												...
												.join(', ')
										"
										@input="v$.na_dry.$touch"
										@blur="v$.na_dry.$touch"
									></v-text-field>
								</div>
								<div class="form-group mb-0">
									<v-text-field
										v-model="data.page.values.obj.k_dry"
										label="Dry deposition of potassium (k)"
										type="number"
										step="any"
										suffix="kg/ha"
										:error-messages="
								...
								<div class="form-group mb-0">
									<v-text-field
										v-model="data.page.values.obj.cl_dry"
										label="Dry deposition of chlorine (cl)"
										type="number"
										step="any"
										suffix="kg/ha"
										:error-messages="
											v$.cl_dry.$errors
												...
												.join(', ')
										"
										@input="v$.cl_dry.$touch"
										@blur="v$.cl_dry.$touch"
									></v-text-field>
								</div>
								<div class="form-group mb-0">
									<v-text-field
										v-model="data.page.values.obj.co3_dry"
										label="Dry deposition of carbonate (co3)"
										type="number"
										step="any"
										suffix="kg/ha"
										:error-messages="
											v$.co3_dry.$errors
												...
												.join(', ')
										"
										@input="v$.co3_dry.$touch"
										@blur="v$.co3_dry.$touch"
									></v-text-field>
								</div>
								<div class="form-group mb-0">
									<v-text-field
										v-model="data.page.values.obj.hco3_dry"
										label="Dry deposition of hydrogen carbonate (hco3)"
										type="number"
										step="any"
										suffix="kg/ha"
										:error-messages="
											v$.hco3_dry.$errors
												...
												.join(', ')
										"
										@input="v$.hco3_dry.$touch"
										@blur="v$.hco3_dry.$touch"
									></v-text-field>
								</div>
							</v-col>
						</v-row>
					...
					<v-card-actions>
						<v-btn
							@click="saveValues"
							:loading="data.page.values.form.saving"
							color="primary"
							variant="text"
							>Save Changes</v-btn
						>
						<v-btn @click="data.page.values.form.show = false"
							>Cancel</v-btn
						>
					</v-card-actions>
				</v-card>
			...
			<v-dialog
				v-model="data.page.values.delete.show"
				:max-width="constants.dialogSizes.md"
			>
				<v-card title="Confirm delete">
					<v-card-text>
						<error-alert
							:text="data.page.values.delete.error"
						></error-alert>

						<p>
							Are you sure you want to delete
							<strong>{{ data.page.values.delete.name }}</strong
					...
					<v-card-actions>
						<v-btn
							@click="confirmDelete"
							:loading="data.page.values.delete.saving"
							color="error"
							variant="text"
							>Delete</v-btn
						>
						<v-btn @click="data.page.values.delete.show = false"
							>Cancel</v-btn
						>
					</v-card-actions>
				</v-card>
