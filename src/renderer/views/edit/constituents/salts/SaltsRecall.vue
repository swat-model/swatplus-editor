...
async function confirmDelete() {
	...
	try {
		...
		data.delete.error = errors.logError(
			error,
...
async function createTemplates() {
	...
	try {
		...
		data.createTemplates.error = errors.logError(
			error,
	...
	<project-container :loading="data.page.loading">
		<div v-if="route.name == 'ConstituentsSaltsRecall'">
			...
			<v-form @submit.prevent="save">
				<error-alert
					as-popup
					v-model="data.page.showError"
					:show="data.page.showError"
				...
				<div class="form-group">
					<v-select
						label="Turn salt point sources module on/off"
						v-model="data.recall"
						:items="data.options"
					>
			...
			>
				<template v-slot:actions>
					...
					<v-btn
						variant="flat"
						color="info"
						@click="data.createTemplates.show = true"
						class="mr-2"
						>Create Templates</v-btn
					>
					<v-btn
						v-if="table.total > 0"
						variant="flat"
				...
				<v-card title="Import/Export Data">
					<v-card-text>
						...
						<div v-else-if="formatters.isNullOrEmpty(task.error)">
							<p>
								Use the toggle below to import time series CSV
								files, or to export your existing data to a
								directory.
							</p>

							<v-btn-toggle
								v-model="data.import.type"
								color="primary"
								variant="outlined"
								mandatory
								class="mb-4"
							...
							<div v-if="data.import.type === 'import_csv'">
								...
								>
									<p>
										...
										file for each recall object.
										<strong
											>Each file should be named for the
											object's name with
											<code>salt_</code> at the beginning,
											e.g., object
											<code>pt001</code> should have a
											file in the directory
											<code>salt_pt001.csv</code>.</strong
										>
									</p>
									<p>
									...
									<p class="mb-0">
										...
										desire, use the
										<strong>Create Templates</strong> button
										at the bottom of the page, then return
										to this form and export the data to CSV.
									</p>
								</v-alert>

								<select-folder-input
									v-model="data.import.inputDir"
									:value="data.import.inputDir"
									class="mb-3"
									label="Directory containing your salt point sources CSV files"
									:required="
										data.import.type === 'import_csv'
									"
							...
							<div v-else>
								<select-folder-input
									v-model="data.import.exportDir"
									:value="data.import.exportDir"
									class="mb-3"
									label="Directory to save your salt point sources CSV files"
									:required="
										data.import.type === 'export_csv'
									"
									invalidFeedback="Please select a folder"
								></select-folder-input>
							</div>
			...
			>
				<v-card title="Data Exported">
					<v-card-text>
						...
						<p>
							<open-file
								:file-path="data.import.exportDir"
								text="Open directory"
								button
								color="primary"
							></open-file>
				...
				<v-card title="Confirm delete">
					<v-card-text>
						...
						<p>
							...
							<strong>ALL</strong> salt point source data? This
			...
			<v-dialog
				v-model="data.createTemplates.show"
				:max-width="constants.dialogSizes.md"
			>
				<v-card title="Create Data Templates">
					<v-card-text>
						<error-alert
							:text="data.createTemplates.error"
						></error-alert>

						<p>
							Create salt point source templates for the selected
							time step.
							<span class="text-error">WARNING:</span> This will
							replace any existing data. To change an individual
							point source object instead, click a row in the
							table and change the time step as needed.
						</p>

						<v-select
							label="Time step"
							v-model="data.createTemplates.time_step"
							:items="data.createTemplates.options.time_step"
						></v-select>
					</v-card-text>
					<v-divider></v-divider>
					<v-card-actions>
						<v-btn
							...
							:loading="data.createTemplates.saving"
							color="primary"
							variant="text"
							>Create Templates</v-btn
						>
						<v-btn @click="data.createTemplates.show = false"
							>Cancel</v-btn
						>
					</v-card-actions>
				</v-card>
