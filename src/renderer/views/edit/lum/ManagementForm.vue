...
let data: any = reactive({
	days: [{ value: 0, title: "None (use heat units)" }],
	months: [
	...
	builder: {
		...
		tables: {
			cropRotation: [
				{
					value: "pl_hv_summer1",
					title: "plant and harvest for continuous summer crop",
				},
				{
					value: "pl_hv_summer2",
					title: "plant and harvest for 2 year summer crop rotation",
				},
				{
					value: "pl_hv_winter1",
					title: "plant and harvest for continuous winter crop",
				},
				{
					value: "pl_hv_ccsws",
					title: "corn-corn-soybean-winter wheat-soybean rotation",
				},
			],
			tillage: [
				{
					value: "fall_plow",
					title: "fall plow followed by field cultivator with spring field cultivator",
				},
				{
					value: "spring_plow",
					title: "spring plow followed by field cultivator with fall field cultivator",
				},
				{
					value: "mulch_till_1",
					title: "mulch tillage - disk before plant and chisel plow after harvest",
				},
				{
					value: "mulch_till_2",
					title: "mulch tillage - chisel plow and disk before planting - no fall tillage",
				},
				{
					value: "no_till",
					title: "no tillage - use zerotill in tillage.til to simulate a planter",
				},
			],
			fertilizer: [
		...
		irrWaterStressRes: [
			...
			{
				value: "irr_str8_r_a_u",
				title: "aquifer backup (and unlimited backup to both)",
			},
			{
				value: "irr_str8_res_aqu",
				title: "aquifer backup (no unlimited backup)",
			},
		],
		irrWaterStressAqu: [
			{ value: "irr_str8_aqu", title: "none" },
			{
				value: "irr_str8_a_r_u",
				title: "reservoir backup (and unlimited backup to both)",
			},
			{
				value: "irr_str8_aqu_res",
				title: "reservoir backup (no unlimited backup)",
			},
		],
		customDescriptions: {
...
function putDb(data: any) {
	if (props.isUpdate) return api.put(`lum/mgt_sch/${props.item.id}`, data, currentProject.getApiHeader());
	else return api.post(`lum/mgt_sch`, data, currentProject.getApiHeader());
}
...
async function save() {
	...
	try {
		...
		else router.push({ name: "Management" });
	} catch (error) {
...
function addAutoOp() {
	...
	let matches = page.form.auto_ops.filter(function (el: any) {
		return el.name === newName;
	});
	let doesNotExist = matches === undefined || matches.length < 1;
	if (data.new_auto_op.name != "" && doesNotExist) {
		if (newName === data.reserved_d_tables.summer1 || newName === data.reserved_d_tables.summer2 || newName === data.reserved_d_tables.winter1) {
	...
	if (!doesNotExist) data.new_auto_op = resetAutoOp;
}
...
function addReservedAutoOp() {
	if (
		formatters.isNullOrEmpty(data.new_auto_op.plant1) ||
...
function sortOps() {
	function compare(a: any, b: any) {
		...
		if (a.hu_sch == 0 && b.hu_sch > 0) return 1;
		if (a.hu_sch > 0 && b.hu_sch == 0) return -1;

		if (a.hu_sch < b.hu_sch) return -1;
		if (a.hu_sch > b.hu_sch) return 1;
...
function isBuilderTableIn(type: string) {
	...
	let matches = array.filter(function (el: any) {
		return tableName.startsWith(el.value);
	});
	return matches && matches.length > 0;
}

function setActionObjNumFromCond(value: any, obj: any) {
	let matches = data.builder.modal.op.table.actions.filter(function (el: any) {
		return el.obj === obj;
	});
...
const validCropRotationConditions = computed(() => {
	return data.builder.modal.op.table.conditions.filter(function (el: any) {
		return ["phu_base0", "phu_plant"].includes(el.var);
	});
});

const validIrrigationConditions = computed(() => {
	return data.builder.modal.op.table.conditions.filter(function (el: any) {
		return ["soil_water", "w_stress", "vol", "aqu_dep"].includes(el.var);
	});
});

const validIrrigationActions = computed(() => {
	return data.builder.modal.op.table.actions.filter(function (el: any) {
		return ["irr_demand", "irrigate"].includes(el.act_typ);
	});
});

const validFertilizerConditions = computed(() => {
	return data.builder.modal.op.table.conditions.filter(function (el: any) {
		return !["plant_gro"].includes(el.var);
	});
});
...
const validFertilizerActions = computed(() => {
	if (!formatters.isNullOrEmpty(data.builder.modal.op.name)) {
		if (data.builder.modal.op.name.startsWith("fert_stess_test")) {
			return [
				{
					heading: "fertilize when nitrogen stress < amount above and plant is growing",
					action: data.builder.modal.op.table.actions[0],
				},
				{
					heading: "fertilize when phosphorus stress < amount above and plant is growing",
					action: data.builder.modal.op.table.actions[1],
				},
			];
		} else if (data.builder.modal.op.name.startsWith("fert_sprg_side")) {
			return [
				{
					heading: "fertilize just before planting in rotation year",
					action: data.builder.modal.op.table.actions[0],
				},
				{
					heading: "fertilize when plant is growing in rotation year, before it reaches maturity",
					action: data.builder.modal.op.table.actions[1],
				},
				{
					heading: "fertilize when plant is growing before it reaches maturity",
					action: data.builder.modal.op.table.actions[2],
				},
			];
		}
	}
...
function getItemsForCond(objType: string) {
	if (objType === "aqu") return data.aquifers;
	if (objType === "res") return data.reservoirs;
	return data.channels;
}

function getObjLabel(type: string): string {
	const labels: Record<string, string> = {
		aqu: "Aquifer",
		res: "Reservoir",
		cha: "Channel",
	};
	return labels[type] ?? "Unlimited";
}

function getObjName(type: string): string {
		...
		<v-form @submit.prevent="save" v-if="!page.loading">
			<div class="form-group">
				<v-text-field
					...
					:rules="[constants.formRules.required, constants.formRules.longNameLength]"
					label="Schedule name"
					hint="Must be unique"
				></v-text-field>
			</div>
				...
				<v-card-item>
					<v-window v-model="page.tabIndex">
						<v-window-item :value="0">
							<v-alert
								v-if="!formatters.isNullOrEmpty(data.builder.warning)"
								type="warning"
								icon="$warning"
								variant="tonal"
								border="start"
							...
							<div v-else class="py-3">
								<p>
									Use the schedule builder below to apply land use decision tables built by the SWAT+ development team. These tables
									are designed cover most use cases, but if you have a complex need, please
									<open-in-browser
										url="https://swat.tamu.edu/contact/"
								...
								<v-alert
									v-if="page.form.operations.length > 0"
									type="warning"
									icon="$warning"
									variant="tonal"
									border="start"
								...
								<div class="mb-6">
									<ul class="list-group">
										<li
											...
											v-for="element in page.form.auto_ops"
											:key="element.name"
										>
											<a href="#" @click.prevent="getBuilderTable(element)"
												><font-awesome-icon :icon="['fas', 'edit']" class="mr-2 text-primary pointer"
													><v-tooltip activator="parent">{{
														constants.commonMessages.leaveWarning
													}}</v-tooltip></font-awesome-icon
												></a
											>
											<div>
												{{ autoOpDescription(element) }}
												{{ autoOpPlantLabel(element) }}
											</div>
											<a href="#" @click.prevent="removeAutoOp(element)" class="ml-auto"
												><font-awesome-icon :icon="['fas', 'times']" class="text-error pointer"
													><v-tooltip activator="parent">Delete</v-tooltip></font-awesome-icon
												></a
											>
										</li>
									</ul>
								...
								<div class="mb-3">
									<v-select
										v-model="data.builder.selections.type"
										label="Select the type of operation..."
										:items="data.builder.typeOptions"
										@update:model-value="data.builder.selections.table = null"
									></v-select>
								</div>
								<div v-if="!formatters.isNullOrEmpty(data.builder.selections.type)">
									<v-select
										v-model="data.builder.selections.table"
										:items="builderTableOptions"
										:label="`Select ${builderTableNullLabel}`"
									></v-select>

									<div class="my-3" v-if="data.builder.selections.table === 'waterStress'">
										<v-select
											v-model="data.builder.selections.irrigation"
											label="Select primary source..."
											:items="data.builder.irrWaterStressSources"
											@update:model-value="data.builder.selections.irrigation2 = null"
										>
										</v-select>

										<div class="my-3" v-if="['waterStressRes', 'waterStressAqu'].includes(data.builder.selections.irrigation)">
											<v-select
												v-model="data.builder.selections.irrigation2"
												label="Select backup source..."
												:items="
													data.builder.selections.irrigation === 'waterStressRes'
														? data.builder.irrWaterStressRes
														: data.builder.irrWaterStressAqu
												"
											>
											</v-select>
										</div>
										...
										<div
											class="mt-3"
											v-if="
												!formatters.isNullOrEmpty(data.builder.selections.irrigation2) ||
												['irr_str8_unlim', 'irr_str8_cha'].includes(data.builder.selections.irrigation)
											"
										>
								...
								<v-dialog v-model="data.builder.modal.show" :max-width="constants.dialogSizes.lg" persistent>
									<v-card :title="data.builder.modal.loading ? 'Loading...' : `Configure: ${data.builder.modal.op.description}`">
										<v-card-text>
											...
											<div v-if="!data.builder.modal.loading">
												...
												<div style="min-height: 300px">
													<div
														v-if="
															!formatters.isNullOrEmpty(data.builder.modal.op.name) &&
															data.builder.modal.op.name.startsWith('pl_hv_')
														"
													>
														<div v-if="reserved_d_table_array.includes(data.builder.modal.op.name)">
															<div class="form-group mb-0">
																<auto-complete
																	...
																	table-name="plant"
																	route-name="PlantsEdit"
																	section="Databases / Plants"
																	help-file="plants.plt"
																	help-db="plants_plt"
																	api-url="db/plants"
																></auto-complete>
															</div>
															...
															<div
																...
																v-if="data.builder.modal.op.name === data.reserved_d_tables.summer2"
															>
																<auto-complete
																	label="Plant 2"
															...
															<v-checkbox
																v-model="data.builder.modal.op.saveAsNew"
																label="Edit heat units?"
																hide-details
															></v-checkbox>
															<div v-if="data.builder.modal.op.saveAsNew">
																<v-table class="table-editor mb-4" density="compact">
																	<thead>
																		<tr class="bg-surface">
																			<th colspan="2" class="bg-secondary-tonal">
																				Conditions to trigger table
																			</th>
																		</tr>
																	</thead>
																	<tbody>
													...
													<div
														v-else-if="
															isBuilderTableIn('irrigation') ||
															isBuilderTableIn('irrWaterStressSources') ||
															isBuilderTableIn('irrWaterStressRes') ||
															isBuilderTableIn('irrWaterStressAqu')
														"
													>
														<v-table class="table-editor" density="compact">
															...
															<tbody>
																<tr v-for="cond in validIrrigationConditions" :key="cond.id">
																	<td class="field">
																		<v-select
																			v-if="['aqu', 'res', 'cha'].includes(cond.obj)"
																			hide-details
																			v-model="cond.obj_num"
																			label="Select..."
																			:items="getItemsForCond(cond.obj)"
																			@update:model-value="setActionObjNumFromCond(cond.obj_num, cond.obj)"
																		></v-select>
																		<v-text-field
																			density="compact"
																			:rules="[constants.formRules.required]"
																	...
																	<td>
																		...
																		<div
																			class="text-error"
																			v-if="cond.obj === 'res' && data.reservoirs.length < 1"
																		>
																			Your project does not contain any reservoirs. You should select a
																			different table.
																		</div>
																	</td>
																</tr>
														...
														<v-table
															class="table-editor"
															density="compact"
															v-for="act in validIrrigationActions"
															:key="act.id"
														>
															<thead>
																<tr class="bg-surface">
																...
																<tr v-if="['aqu', 'res', 'cha'].includes(act.obj)">
																	<td class="field">
																		<v-select
																			...
																			label="Select..."
																			hide-details
																			:items="getItemsForCond(act.obj)"
																		>
																		</v-select>
																	</td>
																	<td>
																		{{ getObjName(act.obj) }}
																		<div class="text-danger" v-if="act.obj === 'res' && !data.reservoirs.length">
																			Your project does not contain any reservoirs. You should select a
																			different table.
																		</div>
																	</td>
													...
													<div v-else-if="isBuilderTableIn('fertilizer')">
														...
														<v-table
															class="table-editor"
															density="compact"
															v-for="act in validFertilizerActions"
															:key="act.heading"
														>
															<thead>
																<tr class="bg-surface">
															...
															<tbody>
																<tr>
																	<td class="field">
																		<v-select
																			v-model="act.action.option"
																			:items="data.fertilizers"
																			hide-details
																		></v-select>
																	</td>
																	<td>
																		fertilizer
																		<reference-popup
																			section="Databases / Fertilizer"
																			help-file="fertilizer.frt"
																			help-db="fertilizer_frt"
																...
																<tr>
																	<td class="field">
																		<v-text-field
																			density="compact"
																			:rules="[constants.formRules.required]"
																			v-model.number="act.action.const"
																			type="number"
																			step="any"
																			hide-details="auto"
																		></v-text-field>
																...
																<tr>
																	<td class="field">
																		<v-text-field
																			density="compact"
																			:rules="[constants.formRules.required]"
																			v-model.number="act.action.const2"
																			type="number"
																			step="any"
																			hide-details="auto"
																		></v-text-field>
																...
																<tr>
																	<td class="field">
																		<v-select
																			v-model="act.action.fp"
																			:items="data.chem_apps"
																			hide-details
																		></v-select>
																	</td>
																	<td>
																		application
																		<reference-popup
																			section="Operations / Chemical Applications"
																			help-file="chem_app.ops"
																			help-db="chem_app_ops"
																			api-url="ops/chemapp"
																		></reference-popup>
																	</td>
																</tr>
										...
										<v-card-actions>
											<v-btn
												v-if="
													formatters.isNullOrEmpty(data.builder.modal.error) &&
													!formatters.isNullOrEmpty(data.builder.modal.op.name) &&
													data.builder.modal.op.name.startsWith('pl_hv_') &&
													!data.builder.modal.op.saveAsNew
												"
												type="button"
												color="primary"
												variant="text"
												@click="saveBuilderTable(true)"
												:loading="data.builder.modal.saving"
												:disabled="data.builder.modal.saving"
											>
												Save
											</v-btn>
											<v-btn
												v-else-if="formatters.isNullOrEmpty(data.builder.modal.error)"
												color="primary"
												variant="text"
												:disabled="data.builder.modal.saving"
											>
												...
												<v-menu activator="parent">
													<v-list>
														<v-list-item @click="saveBuilderTable(true)"
															>Save changes for all uses of this table</v-list-item
														>
														<v-list-item @click="saveBuilderTable(false)"
															>Save changes for only this schedule (makes a new copy)</v-list-item
														>
													</v-list>
												</v-menu>
											</v-btn>
						...
						<v-window-item :value="1">
							<p>
								For most users we recommend using the schedule builder tab to select from decision tables built and tested by the
								model team. Advanced users can select automatic schedules below or enter manual operations.
							</p>
							...
							<v-row>
								<v-col cols="12" md="5">
										...
										<div>
											<ul class="list-group">
												<li
													...
													:key="element.name"
												>
													<a href="#" aria-label="Leave page and edit this item" @click.prevent="goToAutoOp(element.name)"
														><font-awesome-icon
															:icon="['fas', 'edit']"
															class="mr-2 text-primary pointer"
															:title="constants.commonMessages.leaveWarning"
													/></a>
													<div>
														{{ element.name }}
														{{ autoOpPlantLabel(element) }}
													</div>
													<a href="#" aria-label="Delete this item" @click.prevent="removeAutoOp(element)" class="ml-auto"
									...
									<div class="form-group mb-0">
										<auto-complete
											label="Add an automatic schedule"
											v-model="data.new_auto_op.name"
											:value="data.new_auto_op.name"
											table-name="lum.dtl"
											section="Decision Tables / Land Use Management"
											help-file="lum.dtl"
											help-db="d_table_dtl"
											api-url="decision_table/tables/lum.dtl"
										></auto-complete>
									</div>
									<div class="mb-4">
										<v-btn
											...
											color="primary"
											class="wide ml-1"
											:disabled="formatters.isNullOrEmpty(data.new_auto_op.name)"
											@click="addAutoOp"
											>Add</v-btn
										>
									</div>
									...
									<v-dialog v-model="page.reservedTableModal.show" :max-width="constants.dialogSizes.lg" persistent>
										<v-card :title="'Specify plant for ' + data.new_auto_op.name">
											...
											<v-divider></v-divider>
											<v-card-actions>
												<v-btn type="button" color="primary" variant="text" @click="addReservedAutoOp"> Save </v-btn>
								...
								<v-col cols="12" md="6" offset-md="1">
									...
									<div v-if="page.form.operations.length > 0">
										<v-table class="data-table">
											...
											<tbody>
												<tr v-for="(op, i) in sortedOperations" :key="i">
													<td>
														<a href="#" aria-label="Edit this operation" @click.prevent="editOp(op)"
															><font-awesome-icon :icon="['fas', 'edit']" class="text-primary pointer" title="Edit"
														/></a>
													</td>
													<td>{{ op.op_typ }}</td>
													<td>{{ op.order }}</td>
													...
													<td>{{ op.op_data3 }}</td>
													<td>
														<a href="#" aria-label="Delete this operation" @click.prevent="removeOp(op)"
															><font-awesome-icon :icon="['fas', 'times']" class="text-error pointer" title="Delete"
														/></a>
													</td>
												</tr>
										...
										<p>Operation sorting is done by year of rotation followed by heat units, then month/day if heat units = 0.</p>
									</div>
									<div>
										...
										<v-card :title="(page.modal.new ? 'Add' : 'Edit') + ' Operation'">
											<v-card-text>
												...
												<div>
													<v-select
														label="Operation"
														v-model="page.modal.operation.op_typ"
														:items="data.op_codes"
														item-title="text"
														item-value="value"
												...
												<div v-if="page.modal.operation.op_typ == 'skip'">
													In order to sort your skip operation properly, enter a month/day or heat units accordingly. These
													values are not used in the model for skips and are for sorting only.
												</div>

												<v-row>
													<v-col cols="12" md="3">
														<v-text-field
															label="Year of Rotation"
															v-model.number="page.modal.operation.order"
															type="number"
														></v-text-field>
													</v-col>
													<v-col cols="12" md="3">
													...
													<v-col cols="12" md="3">
														<v-text-field
															label="Heat Units Fraction"
															v-model.number="page.modal.operation.hu_sch"
															type="number"
															step="any"
															min="0"
															max="1"
												...
												<v-row v-if="page.modal.operation.op_typ != 'prtp' && page.modal.operation.op_typ != 'skip'">
													<v-col cols="12" md="6">
														<div class="form-group mb-0">
															<auto-complete
																:label="data.op_data1.label"
																v-model="page.modal.operation.op_data1"
																:value="page.modal.operation.op_data1"
																:table-name="getNamesOpData1()"
																:section="data.op_data1.section"
																:help-file="data.op_data1.file"
																:help-db="data.op_data1.db"
																:api-url="getApiUrlOpData1()"
															></auto-complete>
														</div>
													</v-col>
													<v-col
														...
														md="6"
														v-if="
															page.modal.operation.op_typ == 'harv' ||
															page.modal.operation.op_typ == 'hvkl' ||
															page.modal.operation.op_typ == 'fert' ||
															page.modal.operation.op_typ == 'pest'
														"
													>
														<div class="form-group mb-0">
															<auto-complete
																:label="data.op_data2.label"
																v-model="page.modal.operation.op_data2"
																:value="page.modal.operation.op_data2"
																:table-name="getNamesOpData2()"
																:section="data.op_data2.section"
																:help-file="data.op_data2.file"
																:help-db="data.op_data2.db"
																:api-url="getApiUrlOpData2()"
															></auto-complete>
														</div>
													</v-col>
													<v-col cols="12" md="6" v-if="data.has_op_data3.includes(page.modal.operation.op_typ)">
														<v-text-field
															:label="opData3Label"
															v-model.number="page.modal.operation.op_data3"
															type="number"
															step="any"
														></v-text-field>
													</v-col>
											...
											<v-card-actions>
												<v-btn
													...
													variant="text"
													@click="saveOp"
													:disabled="page.modal.saving"
													:loading="page.modal.saving"
												>
													Save
												</v-btn>
