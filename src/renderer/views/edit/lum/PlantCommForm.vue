...
function putDb(formData: any) {
	if (props.isUpdate) return api.put("init/plant_ini/" + props.item.id, formData, currentProject.getApiHeader());
	else return api.post("init/plant_ini", formData, currentProject.getApiHeader());
}
...
async function save() {
	...
	if (!valid) {
		...
		try {
			...
			else router.push({ name: "PlantCommEdit", params: { id: response.data.id } });
		} catch (error) {
...
async function savePlant() {
	...
	if (validatePlant()) {
		try {
			...
			data.plants.form.error = errors.logError(
				error,
				"Invalid plant name, " + data.plants.plnt_name + ". Please make sure it exists in your database.",
			);
		}
					...
					<v-card>
						...
						>
							...
							<template v-slot:item.plnt_name="{ value }">
								<router-link
									v-if="!formatters.isNullOrEmpty(value)"
									:to="`/edit/db/plants/edit/${value.id}`"
				...
				<v-card-text>
					...
					<v-form :validated="data.plants.form.validated">
						<div class="form-group">
							<auto-complete
								label="Plant"
								v-model="data.plants.plnt_name"
								:value="data.plants.plnt_name"
								:show-item-link="data.plants.form.update"
								table-name="plant"
						...
						<v-table class="table-editor" density="compact">
							...
							<tbody>
								<tr-var-editor
									...
									required
									show-range
									v-model="data.plants.obj[v.name]"
									:value="data.plants.obj[v.name]"
									:var-def="v"
								></tr-var-editor>
							</tbody>
						</v-table>
