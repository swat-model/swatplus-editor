...
async function get() {
	...
	let dbType =
		route.params.dbtype === undefined ? "project" : route.params.dbtype;
	console.log(route.params.id);

	try {
		let apiUrl = `decision_table/table/${tableId}`;
		if (dbType === "datasets")
			apiUrl = `decision_table/dataset-table/${tableId}`;
		...
		page.error = errors.logError(
			error,
			"Unable to get decision table from database.",
		);
	}
...
function setTabularItem() {
	...
	let tableDescription = formatters.isNullOrEmpty(page.item.description)
		? ""
	...
	page.tabularItem =
		...
		"acts".padStart(int_pad, " ") +
		tableDescription +
		"\n";

	let numAlts =
		page.item.conditions.length > 0
			? page.item.conditions[0].alts.length
			: 0;

	page.tabularItem +=
		page.item.name.padEnd(str_pad, " ") +
	...
	for (let c of page.item.conditions) {
		...
		let cd = formatters.isNullOrEmpty(c.description)
			? ""
			: `     !${c.description}`;
		page.tabularItem += cd + "\n";
	}
		...
		try {
			if (page.tab === "advanced") {
				...
				const response = await api.put(
					`decision_table/table/${route.params.id}`,
					data,
			...
			page.saveError = errors.logError(
				error,
...
async function copy() {
	...
	if (formatters.isNullOrEmpty(page.copy.name)) {
		...
		try {
			...
			router.push({
				name: "DecisionsEdit",
				params: { id: response.data.id, dbtype: "project" },
			});
			page.copy.show = false;
		} catch (error) {
			page.copy.error = errors.logError(
				error,
				"Unable to save changes to database.",
			);
		}
...
const notEditable = computed(() => {
	return (
		page.reservedTables.includes(page.item.name) ||
		route.params.dbtype === "datasets"
	);
});

const isUsed = computed(() => {
	return (
		(page.item.management_sch_auto_set &&
			page.item.management_sch_auto_set.length > 0) ||
		(page.item.reservoir_res_set &&
			page.item.reservoir_res_set.length > 0) ||
...
<template>
	<project-container :loading="page.loading" :load-error="page.error">
		<file-header
			:input-file="page.item.file_name"
			docs-path="decision-tables"
		>
			<router-link :to="'/edit/decision-table/' + page.linkBack"
				>Decision Tables</router-link
			>
			/ Edit
		</file-header>
		...
		<v-form :validated="page.validated" @submit.prevent="save">
			<div class="form-group">
				<v-text-field
					v-model="page.item.name"
					:rules="[
						constants.formRules.required,
						constants.formRules.nameLength,
					]"
					label="Name"
					hint="Must be unique"
			...
			<v-alert
				v-if="notEditable && route.params.dbtype === 'project'"
				type="info"
				icon="$info"
				variant="tonal"
				border="start"
				class="mb-4"
			>
				This decision table is reserved in the model and cannot be
				modified.
				<br />To change any values in this table, we recommend making a
				copy and editing the copy instead.
			</v-alert>
			<v-alert
				v-if="notEditable && route.params.dbtype === 'datasets'"
				type="info"
				icon="$info"
				variant="tonal"
				border="start"
				class="mb-4"
			>
				This decision table is located in your datasets library. Make a
				copy to add it to your project.
			</v-alert>
			...
			<v-card>
				<v-tabs v-model="page.tab" bg-color="primary">
					...
					<v-tab value="advanced">{{
						notEditable ? "Plain Text" : "Advanced Editing"
					}}</v-tab>
				</v-tabs>
					...
					<v-window v-model="page.tab">
						<v-window-item value="basic">
							<ul class="list-group">
								<li
									...
									v-for="(d, i) in page.descriptions"
									:key="i"
								>
									If {{ d.conditions.join(" and ") }} then
									{{ d.actions.join(" and ") }}
								</li>
							</ul>
							...
							>
								<p class="mb-0">
									We recommend editing this table using the
									<router-link to="/edit/lum/mgt"
										>management schedule
										builder</router-link
								...
								<ul
									v-for="sch in page.item
										.management_sch_auto_set"
									:key="sch.id"
								>
									<li>
										<router-link
											:to="`/edit/lum/mgt/edit/${sch.management_sch.id}`"
											>{{
												sch.management_sch.name
											}}</router-link
										>
									</li>
								</ul>
							...
							>
								...
								<ul
									v-for="res in page.item.reservoir_res_set"
									:key="res.id"
								>
									<li>
										<router-link
											:to="`/edit/cons/reservoirs/edit/${res.id}`"
							...
							<div
								v-else-if="
									page.item.wetland_wet_set &&
									page.item.wetland_wet_set.length > 0
								"
							>
								<p class="mb-0">
									This table is used in the following
								...
								<ul
									v-for="res in page.item.wetland_wet_set"
									:key="res.id"
								>
									<li>
										<router-link
											:to="`/edit/cons/reservoirs/wetlands/edit/${res.id}`"
											>{{ res.name }}</router-link
										>
									</li>
								</ul>
							...
							<div
								v-else-if="
									page.item.hru_lte_hru_set &&
									page.item.hru_lte_hru_set.length > 0
								"
							>
								<p class="mb-0">
									This table is used in the following HRUs:
								</p>
								<ul
									v-for="hru in page.item.hru_lte_hru_set"
									:key="hru.id"
								>
									<li>
										<router-link
											:to="`/edit/cons/hrus-lte/edit/${hru.id}`"
											>{{ hru.name }}</router-link
										>
									</li>
								</ul>
							</div>
							<div v-else>
								<p>
									This table isn't being using in your
									project.
									<span
										v-if="page.item.file_name === 'lum.dtl'"
									>
										Use the
										<router-link to="/edit/lum/mgt"
											>management schedule
											builder</router-link
										>
										to include and/or edit this table.
									</span>
								</p>
							</div>
						</v-window-item>
						...
						<v-window-item value="advanced">
							<v-alert
								...
								class="mb-4"
							>
								Warning: advanced users only. If you are
								comfortable with the decision table format, you
								may edit in plain text below.
							</v-alert>
		...
		<v-dialog
			v-model="page.copy.show"
			size="lg"
			title="Copy Decision Table"
		...
		>
			<v-card title="">
				<v-card-text>
					...
					<p>
						Would you like to make a copy of this table? Enter a
						name for the copy below.
						<br /><span class="text-danger">CAUTION:</span> If the
						table name entered already exists in your project it
						will be overwritten.
					</p>

					<div class="form-group">
						<v-text-field
							v-model="page.copy.name"
							:rules="[
								constants.formRules.required,
								constants.formRules.nameLength,
							]"
