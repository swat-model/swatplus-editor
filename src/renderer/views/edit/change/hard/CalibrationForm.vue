...
const itemRules = computed(() => ({
	...
	soil_lyr1: {
		...
		required: helpers.withMessage(
			"Value is required",
			requiredIf(() => {
				return data.parmTypes.sol.includes(props.item.cal_parm_name);
			}),
		),
	},
	soil_lyr2: {
	...
	yr1: {
		...
		required: helpers.withMessage(
			"Value is required",
			requiredIf(() => {
				return data.parmTypes.cli.includes(props.item.cal_parm_name);
			}),
		),
	},
	yr2: {
...
let data: any = reactive({
	...
	chgTypeOptions: [
		{
			value: "absval",
			title: "Change the value of the parameter (absval)",
		},
		{
			value: "abschg",
			title: "Change the value by the specified amount (abschg)",
		},
		{
			value: "pctchg",
			title: "Change the value by the specified percent (pctchg)",
		},
	],
...
function putDb(formData: any) {
	if (props.isUpdate)
		return api.put(
			`change/calibration/${props.item.id}`,
			formData,
...
async function get() {
	...
	try {
		...
		data.page.error = errors.logError(
			error,
			"Unable to load calibration parameters from database.",
		);
	}
		...
		try {
			...
			data.page.error = errors.logError(
				...
				"Invalid parameter name, " +
					props.item.cal_parm_name +
					". Please make sure the calibration parameter exists in your database.",
			);
		}
...
const objTable = computed(() => {
	if (!props.isUpdate || !props.item.cal_parm || !props.item.cal_parm.obj_typ)
		return "";
...
<template>
	<project-container :loading="data.page.loading">
		...
		<success-alert
			v-model="data.page.saveSuccess"
			:show="data.page.saveSuccess"
		></success-alert>
		...
		<v-form @submit.prevent="save">
			<div class="form-group">
				<auto-complete
					:label="
						...
						(props.isUpdate ? ' (Cannot Modify)' : '')
					"
					:disabled="props.isUpdate"
					v-model="item.cal_parm_name"
					:value="item.cal_parm_name"
					:show-item-link="props.isUpdate"
					table-name="cal_parms"
					route-name="HardCalibrationParmsEdit"
					section="Change / Calibration Parameters"
					help-file="cal_parms.cal"
					help-db="cal_parms_cal"
					api-url="change/cal_parms"
				></auto-complete>
			</div>
			...
			<v-row>
				<v-col cols="12" md="6">
					<v-select
						...
						v-model="item.chg_typ"
						:items="data.chgTypeOptions"
						:error-messages="
							v$.chg_typ.$errors.map((e) => e.$message).join(', ')
						"
						@input="v$.chg_typ.$touch"
						@blur="v$.chg_typ.$touch"
					></v-select>
				</v-col>
				<v-col cols="12" md="6">
					<v-text-field
						label="Value of Change"
						v-model.number="item.chg_val"
						type="number"
						step="any"
						:error-messages="
							v$.chg_val.$errors.map((e) => e.$message).join(', ')
						"
						@input="v$.chg_val.$touch"
						@blur="v$.chg_val.$touch"
					></v-text-field>
				</v-col>
			</v-row>
			...
			<v-row v-if="data.parmTypes.sol.includes(item.cal_parm_name)">
				<v-col cols="12" md="6">
					<v-text-field
						label="First Soil Layer to Apply Change"
						v-model.number="item.soil_lyr1"
						type="number"
						step="any"
						:error-messages="
							v$.soil_lyr1.$errors
								...
								.join(', ')
						"
						@input="v$.soil_lyr1.$touch"
						@blur="v$.soil_lyr1.$touch"
					></v-text-field>
				</v-col>
				<v-col cols="12" md="6">
					<v-text-field
						label="Last Soil Layer to Apply Change"
						v-model.number="item.soil_lyr2"
						type="number"
						step="any"
						:error-messages="
							v$.soil_lyr2.$errors
								...
								.join(', ')
						"
						@input="v$.soil_lyr2.$touch"
						@blur="v$.soil_lyr2.$touch"
					></v-text-field>
				</v-col>
			</v-row>
			...
			<v-row v-if="data.parmTypes.cli.includes(item.cal_parm_name)">
				<v-col cols="12" md="3">
					<v-text-field
						label="First Year to Apply Change"
						v-model.number="item.yr1"
						type="number"
						:error-messages="
							v$.yr1.$errors.map((e) => e.$message).join(', ')
						"
						@input="v$.yr1.$touch"
						@blur="v$.yr1.$touch"
					></v-text-field>
				...
				<v-col cols="12" md="3">
					<v-text-field
						label="Last Year to Apply Change"
						v-model.number="item.yr2"
						type="number"
						:error-messages="
							v$.yr2.$errors.map((e) => e.$message).join(', ')
						"
						@input="v$.yr2.$touch"
						@blur="v$.yr2.$touch"
					></v-text-field>
				...
				<v-col cols="12" md="3">
					<v-text-field
						label="First Day to Apply Change"
						v-model.number="item.day1"
						type="number"
						min="1"
						max="366"
						:error-messages="
							v$.day1.$errors.map((e) => e.$message).join(', ')
						"
						@input="v$.day1.$touch"
						@blur="v$.day1.$touch"
					></v-text-field>
				...
				<v-col cols="12" md="3">
					<v-text-field
						label="Last Day to Apply Change"
						v-model.number="item.day2"
						type="number"
						min="1"
						max="366"
						:error-messages="
							v$.day2.$errors.map((e) => e.$message).join(', ')
						"
						@input="v$.day2.$touch"
						@blur="v$.day2.$touch"
					></v-text-field>
			...
			<div v-if="isUpdate">
				<div v-if="item?.cal_parm?.obj_typ !== 'bsn'">
					...
					<object-selector
						v-if="!formatters.isNullOrEmpty(objTable)"
						hide-alert
						name="Objects"
						:table="objTable"
						:is-hru="objTableIsHru"
						:no-gis="!objTableIsGis"
						:initial-selection="props.initialSelection"
						@change="objectSelectionChange"
					></object-selector>
				</div>
				...
				<v-card v-if="data.conditions.length > 0" class="my-3">
					<v-table class="data-table" density="compact">
						...
						<tbody>
							<tr
								v-for="cond in data.conditions"
								:key="cond.calibration_cal_id"
							>
								<td class="min">
									<a
										@click.prevent="editCond(cond)"
										href="#"
										aria-label="Edit"
										><font-awesome-icon
											:icon="['fas', 'edit']"
			...
			<div class="mt-3" v-if="!isUpdate">
				You will be able to set conditions and objects to which to apply
				the calibration after clicking save changes below.
			</div>

			<action-bar>
				<v-btn
					type="submit"
					:loading="data.page.saving"
					variant="flat"
					color="primary"
					class="mr-2"
				>
				...
				<v-card-text>
					...
					<div class="form-group">
						<v-select
							...
							v-model="data.modal.condition.cond_typ"
							:items="data.conditionTypeOptions"
						></v-select>
					</div>
					...
					<div class="form-group">
						<v-select
							...
							v-model="data.modal.condition.cond_op"
							:items="data.conditionOperatorOptions"
						></v-select>
					</div>
					...
					<div class="form-group">
						<v-text-field
							label="Value"
							v-if="data.modal.condition.cond_typ == 'region'"
							v-model.number="data.modal.condition.cond_val"
							type="number"
							step="any"
						></v-text-field>
						<v-text-field
							label="Value"
							v-else
							v-model="data.modal.condition.cond_val_text"
						></v-text-field>
					</div>
				</v-card-text>
				<v-divider></v-divider>
