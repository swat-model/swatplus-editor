		...
		<div v-if="route.path === '/'">
			...
			<v-main v-else-if="!formatters.isNullOrEmpty(page.error)">
				<div class="py-3 px-6">
					...
					<v-alert color="info" icon="$info">
						<template #text>
							<p>
								SWAT+ Editor encountered an error:
								{{ page.error }}
							</p>
							<p>
				...
				<v-main>
					...
					>
						...
						<swat-plus-toolbox-button
							v-if="versionSupport.supported && !info.is_lte"
							text="SWAT+ Toolbox"
							:ran-swat="info.status.wrote_inputs"
						></swat-plus-toolbox-button>
						...
						<v-btn
							@click="
								page.close.removeFromRecent = false;
								page.close.show = true;
							"
							:active="false"
						>
			...
			>
				<v-card title="Open Project">
					<v-card-text>
						<select-file-input
							v-model="page.open.projectDb"
							:value="page.open.projectDb"
							label="Select your project/GIS SQLite database file"
							file-type="sqlite"
							required
							invalid-feedback="Select a SQLite database file"
						></select-file-input>
					</v-card-text>
					<v-divider></v-divider>
					<v-card-actions>
			...
			>
				<v-card title="Close Current Project?">
					<v-card-text>
						<p>
							This will close the project currently open,
				...
				<v-card title="Start SWAT+ Editor Project from QSWAT+">
					<v-card-text>
						...
						<div v-else-if="formatters.isNullOrEmpty(task.error)">
							...
							<v-form>
								<v-text-field
									v-model="page.import.project.name"
									:rules="[constants.formRules.required]"
									label="Project display name"
								></v-text-field>
			...
			>
				<v-card title="Create a New SWAT+ Editor Project">
					<v-card-text>
						...
						<div v-else-if="formatters.isNullOrEmpty(task.error)">
							...
							<v-form>
								...
								<select-folder-input
									v-model="page.create.projectFolder"
									:value="page.create.projectFolder"
									class="mb-3"
								...
								<select-file-input
									v-model="page.create.datasetsDb"
									:value="page.create.datasetsDb"
									class="mb-3"
									label="Select your SWAT+ datasets SQLite database file"
									hint="This will be copied to your project folder above"
									persistent-hint
									fileType="sqlite"
									required
					...
					<v-card-actions>
						<v-btn
							v-if="formatters.isNullOrEmpty(task.error)"
							:loading="page.create.loading || task.running"
							@click="createProject"
							color="primary"
							variant="text"
							>Create</v-btn
