...
let data = reactive({
	page: {
		...
		tabIndex: 0,
		tabs: ["Overview", "Hydrology", "Nutrients", "Sediment", "Plants", "Point Sources", "Reservoirs"],
		checkByLanduse: false,
		/*selectedLanduse: null as string | null,
			selectedArea: 0,
			selectedHruCount: 0,*/
		...
		selectedHruIndex: null as number | null,
		...
		if (!formatters.isNullOrEmpty(data.config.output_last_imported)) {
			...
			if (response2.data.error) {
				...
				if (!response.data.has_observed_weather) {
					data.simulationWarnings.push(
						"You are using simulated precipitation data; if you intend to calibrate, you should used measured precipitation data",
					);
				}
...
function nextTab(position: number) {
	...
	if (data.page.tabIndex > maxTabPos) data.page.tabIndex = 0;
	if (data.page.tabIndex < 0) data.page.tabIndex = maxTabPos;
}
...
const landuseBackground = computed(() => {
	if (theme.global.name.value === "light" && data.page.checkByLanduse && data.page.selectedLandusesInput.length > 0) {
		let matches = data.check.landuseBackgrounds.filter((l: any) => data.page.selectedLandusesInput.some((lu: any) => l.name === lu));
		if (matches.length > 0) {
...
<template>
	<project-container
		...
		add-error-frame
		loading-message="Reading outputs for SWAT+ Check. This may take a few moments on larger models."
	>
		<v-main>
			...
			<div v-else>
				<v-tabs
					v-model="data.page.tabIndex"
					id="spcheck_tabs"
					align-tabs="center"
					color="primary"
					:bg-color="theme.global.name.value == 'dark' ? 'blue-grey-darken-4' : 'blue-grey-lighten-3'"
				>
				...
				<div v-if="data.page.tabIndex == 0" :class="`spcheck_tab ${theme.global.name.value}`" id="spcheck_overview">
					<v-card style="max-width: 800px" class="semi-transparent mt-8" elevation="6">
						<v-card-item>
							...
							<p>
								SWAT+ Check reads model output from a SWAT+ project and performs many simple checks to identify potential model
								problems. The intended purpose of this program is to identify model problems early in the modeling process. Hidden
								model problems often result in the need to recalibrate or regenerate a model, resulting in an avoidable waste of time.
								This program is designed to compare a variety of SWAT+ outputs to nominal ranges based on the judgment of model
								developers. A warning does not necessarily indicate a problem; the purpose is to bring attention to unusual
								predictions. This software also provides a visual representation of various model outputs to aid novice users.
							</p>

							<v-alert v-if="data.check.info.gwflow" type="warning" icon="$warning" variant="tonal" border="start" class="my-4">
								<p>
									Your model is using the GWFLOW module. SWAT+ Check is not fully compatible with GWFLOW at this time. We will
									update this as soon as possible. For now the following values are unavailable:
								</p>
								<ul>
								...
								<p>We encourage you to look in the GWFLOW output files on your own until a fix is available.</p>
							</v-alert>
									...
									<v-table small density="compact" class="transparent">
										<tbody>
											<tr>
												...
												<td>
													SWAT+
													{{ data.check.info.swatVersion }}
												</td>
											</tr>
											<tr>
												<th>Simulation Length</th>
												<td>
													{{ data.check.info.simulationLength }}
													years
												</td>
											</tr>
											<tr>
									...
									<v-table small density="compact" class="transparent">
										<tbody>
											<tr>
												...
												<td>
													{{ formatters.toNumberFormat(data.check.info.watershedArea, 3) }}
													ha
												</td>
											</tr>
											<tr>
												<th>HRU Land Area</th>
												<td>
													{{ formatters.toNumberFormat(data.check.info.hruTotalArea, 3) }}
													ha
												</td>
											</tr>
											<tr>
												<th>HRUs</th>
												<td>
													{{ formatters.toNumberFormat(data.check.info.hrus, 0) }}
												</td>
											</tr>
											<tr>
												<th>LSUs</th>
												<td>
													{{ formatters.toNumberFormat(data.check.info.lsus, 0) }}
												</td>
											</tr>
											<tr>
												<th>Subbasins</th>
												<td>
													{{ formatters.toNumberFormat(data.check.info.subbasins, 0) }}
												</td>
											</tr>
										</tbody>
									</v-table>
				...
				<image-overlays
					v-if="data.page.tabIndex == 1"
					class="spcheck_tab"
					id="spcheck_hydrology"
				...
				>
					<template #mainContent>
						<v-card class="semi-transparent details-card" elevation="6">
							<v-card-item>
								...
								<v-select
									v-if="data.page.checkByLanduse && data.check.landuseCategories.length > 0"
									hide-details
									density="comfortable"
									:items="data.check.landuseCategories"
									item-title="name"
									item-value="name"
									label="Filter by category"
									v-model="data.page.selectedCategory"
									@update:model-value="setLanduseInCategory"
								>
								</v-select>
								<v-autocomplete
									v-if="data.page.checkByLanduse"
									hide-details
									density="comfortable"
									multiple
									chips
									closable-chips
									v-model="data.page.selectedLandusesInput"
									:items="data.check.landuseOptions"
									label="Land use"
								...
								<div v-if="data.page.checkByLanduse && data.page.selectedLandusesInput.length > 0" class="mt-2 text-subtitle-2">
									<div v-for="(lu, index) in data.page.selectedLanduses" :key="index">
										{{ lu.landuse }} /
										{{ formatters.toNumberFormat(lu.area, 2) }}
										ha /
										{{ formatters.toNumberFormat(lu.hruCount, 0) }}
										HRUs
									</div>
								</div>
								...
								<v-table small density="compact" class="transparent">
									<tbody>
										<tr>
											...
											<td class="text-right">
												{{ formatters.toNumberDecimals(selectedData.etToPrecip, 3) }}
											</td>
										</tr>
										<tr>
											<th>Deep Recharge/Precipitation</th>
											<td class="text-right">
												{{ formatters.toNumberDecimals(selectedData.seepToPrecip, 3) }}
											</td>
										</tr>
										<tr>
											<th>Streamflow/Precipitation</th>
											<td class="text-right">
												{{ formatters.toNumberDecimals(selectedData.totalFlowToPrecip, 3) }}
											</td>
										</tr>
										<tr>
											<th>Baseflow/Total Flow</th>
											<td class="text-right">
												{{ formatters.toNumberDecimals(selectedData.baseflowToTotal, 3) }}
											</td>
										</tr>
										<tr>
											<th>Surface Runoff/Total Flow</th>
											<td class="text-right">
												{{ formatters.toNumberDecimals(selectedData.surfaceflowToTotal, 3) }}
											</td>
										</tr>
										<tr>
											<th>Percolation/Precipitation</th>
											<td class="text-right">
												{{ formatters.toNumberDecimals(selectedData.percoToPrecip, 3) }}
											</td>
										</tr>
									</tbody>
								</v-table>
					...
					<template #hyd_precip>
						...
						<div>
							{{ formatters.toNumberDecimals(selectedData.precip, 3) }}
							mm
						</div>
					</template>
					...
					<template #hyd_et>
						...
						<div>
							{{ formatters.toNumberDecimals(selectedData.et, 3) }}
							mm
						</div>
					</template>
					...
					<template #hyd_pet>
						...
						<div>
							{{ formatters.toNumberDecimals(selectedData.pet, 3) }}
							mm
						</div>
					</template>
					...
					<template #hyd_surq_gen>
						...
						<div>
							{{ formatters.toNumberDecimals(selectedData.surq_gen, 3) }}
							mm
						</div>
					</template>
					...
					<template #hyd_latq>
						...
						<div>
							{{ formatters.toNumberDecimals(selectedData.latq, 3) }}
							mm
						</div>
					</template>
					...
					<template #hyd_irr>
						...
						<div>
							{{ formatters.toNumberDecimals(selectedData.irr, 3) }}
							mm
						</div>
					</template>
					...
					<template #hyd_aqu_revap>
						...
						<div>
							{{ formatters.toNumberDecimals(selectedData.aqu_revap, 3) }}
							mm
						</div>
					</template>
					...
					<template #hyd_perc>
						...
						<div>
							{{ formatters.toNumberDecimals(selectedData.perc, 3) }}
							mm
						</div>
					</template>
					...
					<template #hyd_aqu_flo_cha v-if="data.page.isOverall">
						...
						<div>
							{{ formatters.toNumberDecimals(selectedData.aqu_flo_cha, 3) }}
							mm
						</div>
					</template>
					...
					<template #hyd_aqu_seep v-if="data.page.isOverall">
						...
						<div>
							{{ formatters.toNumberDecimals(selectedData.aqu_seep, 3) }}
							mm
						</div>
					</template>
					...
					<template #hyd_cn>
						...
						<div>
							{{ formatters.toNumberDecimals(selectedData.cn, 3) }}
						</div>
					</template>
				</image-overlays>

				<image-overlays
					v-if="data.page.tabIndex == 2"
					class="spcheck_tab"
					id="spcheck_nutrients"
					...
					:overlays="[
						{ x: 0.65, y: 0.1, slot: 'nut_units' },
						{ x: 0.457, y: 0.1, slot: 'nutn_orgn' },
						{ x: 0.502, y: 0.1, slot: 'nutn_minn' },
						...
						{ x: 0.457, y: 0.1, slot: 'nutp_orgp' },
						{ x: 0.502, y: 0.1, slot: 'nutp_minp' },
				...
				>
					<template #mainContent>
						<v-card class="semi-transparent details-card" elevation="6">
							<v-card-item>
								...
								<v-tabs
									v-model="data.page.nutrientsTab"
									align-tabs="center"
									:class="data.page.checkByLanduse ? 'mt-4' : 'mt-0'"
									color="primary"
									:bg-color="theme.global.name.value == 'dark' ? 'blue-grey-darken-2' : 'blue-grey-lighten-5'"
								>
									...
									<v-table small density="compact" class="transparent">
										<tbody>
											<tr>
												<th>Total N Loss</th>
												<td class="text-right">
													{{ formatters.toNumberDecimals(selectedData.nLossesTotalLoss, 3) }}
												</td>
											</tr>
											<tr>
												<th>Organic N</th>
												<td class="text-right">
													{{ formatters.toNumberDecimals(selectedData.nLossesOrgN, 3) }}
												</td>
											</tr>
											<tr>
												<th>Nitrate Surface Runoff</th>
												<td class="text-right">
													{{ formatters.toNumberDecimals(selectedData.nLossesSurfaceRunoff, 3) }}
												</td>
											</tr>
											<tr>
												<th>Nitrate Lateral Flow</th>
												<td class="text-right">
													{{ formatters.toNumberDecimals(selectedData.nLossesLateralFlow, 3) }}
												</td>
											</tr>
											<tr>
												<th>Solubility Ratio in Runoff</th>
												<td class="text-right">
													{{ formatters.toNumberDecimals(selectedData.nLossesSolubilityRatio, 3) }}
												</td>
											</tr>
										</tbody>
									</v-table>
									...
									<v-table small density="compact" class="transparent">
										<tbody>
											<tr>
												<th>Total P Loss</th>
												<td class="text-right">
													{{ formatters.toNumberDecimals(selectedData.pLossesTotalLoss, 3) }}
												</td>
											</tr>
											<tr>
												<th>Organic P</th>
												<td class="text-right">
													{{ formatters.toNumberDecimals(selectedData.pLossesOrgP, 3) }}
												</td>
											</tr>
											<tr>
												<th>Soluble P Surface Runoff</th>
												<td class="text-right">
													{{ formatters.toNumberDecimals(selectedData.pLossesSurfaceRunoff, 3) }}
												</td>
											</tr>
											<tr>
												<th>Solubility Ratio in Runoff</th>
												<td class="text-right">
													{{ formatters.toNumberDecimals(selectedData.pLossesSolubilityRatio, 3) }}
												</td>
											</tr>
										</tbody>
									</v-table>
								...
								<div class="mt-2">
									<v-btn
										@click="data.modals.reachReport = true"
										variant="tonal"
										size="small"
										append-icon="fas fa-arrow-up-right-from-square"
										>View delivery ratio by channel</v-btn
									>
								</div>
								...
								<div class="warning-list mb-2">
									<ul v-if="data.page.nutrientsTab === 'nitrogen'">
										...
										<li
											v-if="!selectedData.warnings.nb_nitrogen || selectedData.warnings.nb_nitrogen.length === 0"
											class="text-body-2"
										>
											<i>None</i>
										</li>
									...
									<ul v-else>
										...
										<li
											v-if="!selectedData.warnings.nb_phosphorus || selectedData.warnings.nb_phosphorus.length === 0"
											class="text-body-2"
										>
											<i>None</i>
										</li>
					...
					<template #nutp_rsd_laborg_p v-if="data.page.nutrientsTab === 'phosphorus'">
						...
						<div>
							{{ formatters.toNumberDecimals(selectedData.rsd_laborg_p, 3) }}
						</div>
					</template>
					...
					<template #nutp_sedorgp v-if="data.page.nutrientsTab === 'phosphorus'">
						...
						<div>
							{{ formatters.toNumberDecimals(selectedData.sedorgp, 3) }}
						</div>
					</template>
					...
					<template #nutp_pplnt v-if="data.page.nutrientsTab === 'phosphorus'">
						...
						<div>
							{{ formatters.toNumberDecimals(selectedData.pplnt, 3) }}
						</div>
					</template>
					...
					<template #nutp_act_sta_p v-if="data.page.nutrientsTab === 'phosphorus'">
						<div>
							{{ formatters.toNumberDecimals(selectedData.act_sta_p, 3) }}
						</div>
					</template>
					...
					<template #nutp_fertp v-if="data.page.nutrientsTab === 'phosphorus'">
						...
						<div>
							{{ formatters.toNumberDecimals(selectedData.fertp, 3) }}
						</div>
					</template>
					...
					<template #nutn_rsd_nitorg_n v-if="data.page.nutrientsTab === 'nitrogen'">
						<div class="label">Residue Mineralization</div>
						<div>
							{{ formatters.toNumberDecimals(selectedData.rsd_nitorg_n, 3) }}
						</div>
					</template>
					...
					<template #nutn_mineralization>
						...
						<div>
							{{ formatters.toNumberDecimals(selectedData.mineralization, 3) }}
						</div>
					</template>
					...
					<template #nutn_sedorgn v-if="data.page.nutrientsTab === 'nitrogen'">
						<div class="label">Organic Fertilizer</div>
						<div>
							{{ formatters.toNumberDecimals(selectedData.sedorgn, 3) }}
						</div>
					</template>
					...
					<template #nutn_nplt v-if="data.page.nutrientsTab === 'nitrogen'">
						<div class="label">Uptake</div>
						<div>
							{{ formatters.toNumberDecimals(selectedData.nplt, 3) }}
						</div>
					</template>
					...
					<template #nutn_act_sta_n v-if="data.page.nutrientsTab === 'nitrogen'">
						<div class="label">Decay</div>
						<div>
							{{ formatters.toNumberDecimals(selectedData.act_sta_n, 3) }}
						</div>
					</template>
					...
					<template #nutn_denit v-if="data.page.nutrientsTab === 'nitrogen'">
						...
						<div>
							{{ formatters.toNumberDecimals(selectedData.denit, 3) }}
						</div>
					</template>
					...
					<template #nutn_fertn v-if="data.page.nutrientsTab === 'nitrogen'">
						...
						<div>
							{{ formatters.toNumberDecimals(selectedData.fertn, 3) }}
						</div>
					</template>
					...
					<template #nutn_volatilization v-if="data.page.nutrientsTab === 'nitrogen'">
						...
						<div>
							{{ formatters.toNumberDecimals(selectedData.volatilization, 3) }}
						</div>
					</template>
					...
					<template #nutn_fixn v-if="data.page.nutrientsTab === 'nitrogen'">
						...
						<div>
							{{ formatters.toNumberDecimals(selectedData.fixn, 3) }}
						</div>
					</template>
				</image-overlays>

				<image-overlays
					v-if="data.page.tabIndex == 3"
					class="spcheck_tab"
					id="spcheck_sediment"
				...
				>
								...
								<v-table small density="compact" class="transparent">
									<tbody>
										<tr>
											...
											<td class="text-right">
												{{ formatters.toNumberDecimals(data.check.basin.sedyld, 3) }}
												t/ha
											</td>
										</tr>
										<tr>
											...
											<td class="text-right">
												{{ formatters.toNumberDecimals(data.check.basin.sed_out - data.check.basin.sed_in, 3) }}
												t/ha
											</td>
										</tr>
										<tr>
											...
											<td class="text-right">
												{{ formatters.toNumberDecimals(data.check.basin.chaErosion, 3) }}
												%
											</td>
										</tr>
										<tr>
											...
											<td class="text-right">
												{{ formatters.toNumberDecimals(data.check.basin.chaDeposition, 3) }}
												%
											</td>
										</tr>
									</tbody>
					...
					<template #sed_runoff>
						<div class="label">Surface Runoff</div>
						<div>
							{{ formatters.toNumberDecimals(data.check.basin.surq_gen, 3) }}
							mm/yr
						</div>
					</template>
					...
					<template #sed_change>
						...
						<div>
							{{ formatters.toNumberDecimals(data.check.info.hruTotalArea * (data.check.basin.sed_out - data.check.basin.sed_in), 3) }}
							tons
						</div>
					</template>
					...
					<template #sed_yield>
						...
						<div>
							Maximum:
							{{ formatters.toNumberDecimals(data.check.basin.maxUplandSedYield, 3) }}
							t/ha
						</div>
						<div>
							Average:
				...
				<image-overlays
					...
					:overlays="[
						...
						{ x: 0.55, y: 0.2, slot: 'plant_yield_bioms' },
				...
				>
					<template #mainContent>
						<v-card class="semi-transparent details-card" elevation="6">
							<v-card-item>
								...
								<v-table small density="compact" class="transparent">
									<tbody>
										<tr>
											...
											<td class="text-right">
												{{ formatters.toNumberDecimals(selectedData.strstmp, 3) }}
											</td>
										</tr>
										<tr>
											<th>Water Stress Days</th>
											<td class="text-right">
												{{ formatters.toNumberDecimals(selectedData.strsw, 3) }}
											</td>
										</tr>
										<tr>
											<th>Nitrogen Stress Days</th>
											<td class="text-right">
												{{ formatters.toNumberDecimals(selectedData.strsn, 3) }}
											</td>
										</tr>
										<tr>
											<th>Phosphorus Stress Days</th>
											<td class="text-right">
												{{ formatters.toNumberDecimals(selectedData.strsp, 3) }}
											</td>
										</tr>
										<tr>
											<th>Soil Air Stress Days</th>
											<td class="text-right">
												{{ formatters.toNumberDecimals(selectedData.strsa, 3) }}
											</td>
										</tr>
									</tbody>
								</v-table>
								...
								<v-autocomplete
									v-if="filteredMgtOptions && filteredMgtOptions.length > 0"
									hide-details
									class="mb-2"
									density="compact"
									v-model="data.page.selectedHruIndex"
									:items="filteredMgtOptions"
									label="Select an HRU"
									placeholder="Type to search..."
								></v-autocomplete>
						...
						<v-card v-if="selectedHru != null" class="mgt-card" elevation="0" density="compact">
							<v-card-item>
								...
								<div class="text-subtitle-2">
									...
									{{ formatters.toNumberFormat(selectedHru.area, 2) }}
									ha
								</div>
								...
								<div v-else>
									<v-timeline align="start" side="end" density="compact">
										<v-timeline-item
											v-for="(mgt, index) in selectedHru.mgts"
											:key="index"
											:dot-color="getMgtIconAndColor(mgt.op).color"
											:icon="getMgtIconAndColor(mgt.op).icon"
											fill-dot
											size="small"
										>
					...
					<template #plant_nplt_pplnt>
						...
						<div>
							Nitrogen:
							{{ formatters.toNumberDecimals(selectedData.nuptake, 3) }}
							kg/ha
						</div>
						<div>
							Phosphorus:
							{{ formatters.toNumberDecimals(selectedData.puptake, 3) }}
							kg/ha
						</div>
					</template>
					...
					<template #plant_yield_bioms>
						<div>
							Average Yield:
							{{ formatters.toNumberDecimals(selectedData.yield_val, 3) }}
							kg/ha
						</div>
						<div>
							Average Biomass:
							{{ formatters.toNumberDecimals(selectedData.bioms, 3) }}
							kg/ha
						</div>
					</template>
							...
							<v-col cols="12" lg="3">
								<v-card class="semi-transparent" elevation="6">
									<v-card-item>
										...
										<p class="text-body-2">
											Point sources constantly discharge pollutants to streams. These are an optional feature in SWAT+. These
											summaries are presented so that the relative contribution of these sources can be verified. Point sources
											contributions are so varied that there is no reasonable range which can be applied to all basins.
										</p>
										...
										<div class="warning-list mb-2">
											<ul>
												...
												<li
													v-if="!data.check.basin.warnings.ptsrc || data.check.basin.warnings.ptsrc.length === 0"
													class="text-body-2"
												>
													<i>None</i>
												</li>
										...
										<v-table small density="compact" class="my-2 transparent">
											<tbody>
												<tr>
													<th>Flow</th>
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.subbasinLoad.flow, 3) }}
													</td>
													<td>cms</td>
												</tr>
												<tr>
													<th>Sediment</th>
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.subbasinLoad.sediment, 3) }}
													</td>
													<td>Mg/yr</td>
												</tr>
												<tr>
													<th>Nitrogen</th>
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.subbasinLoad.nitrogen, 3) }}
													</td>
													<td>kg/yr</td>
												</tr>
												<tr>
													<th>Phosphorus</th>
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.subbasinLoad.phosphorus, 3) }}
													</td>
													<td>kg/yr</td>
												</tr>
											</tbody>
										...
										<v-table small density="compact" class="my-2 transparent">
											<tbody>
												<tr>
													<th>Flow</th>
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.pointSourceInletLoad.flow, 3) }}
													</td>
													<td>cms</td>
												</tr>
												<tr>
													<th>Sediment</th>
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.pointSourceInletLoad.sediment, 3) }}
													</td>
													<td>Mg/yr</td>
												</tr>
												<tr>
													<th>Nitrogen</th>
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.pointSourceInletLoad.nitrogen, 3) }}
													</td>
													<td>kg/yr</td>
												</tr>
												<tr>
													<th>Phosphorus</th>
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.pointSourceInletLoad.phosphorus, 3) }}
													</td>
													<td>kg/yr</td>
												</tr>
											</tbody>
										...
										<v-table small density="compact" class="my-2 transparent">
											<tbody>
												<tr>
													<th>Flow</th>
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.fromInletAndPointSource.flow, 3) }}
													</td>
													<td>%</td>
												</tr>
												<tr>
													<th>Sediment</th>
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.fromInletAndPointSource.sediment, 3) }}
													</td>
													<td>%</td>
												</tr>
												<tr>
													<th>Nitrogen</th>
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.fromInletAndPointSource.nitrogen, 3) }}
													</td>
													<td>%</td>
												</tr>
												<tr>
													<th>Phosphorus</th>
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.fromInletAndPointSource.phosphorus, 3) }}
													</td>
													<td>%</td>
												</tr>
											</tbody>
							...
							<v-col cols="12" lg="3">
								<v-card class="semi-transparent" elevation="6">
									<v-card-item>
										...
										<p class="text-body-2">
											Reservoirs are an optional feature in SWAT+. The hydrology of basins with large reservoirs may be
											completely dominated by reservoir processes and release rates. The data presented here is an average of
											all reservoirs;
											<a href="#" @click.prevent="data.modals.reservoirs = true">see data for individual reservoirs</a>. The
											statistics presented here are designed to identify common reservoir issues. The use of user specified
											release rate may cause a reservoir to grow continuously or run completely dry. These common issues can be
											detected via the final/initial volume ratio and fraction of period empty statistics below.
										</p>
										...
										<div class="warning-list mb-2">
											<ul>
												...
												<li
													v-if="!data.check.basin.warnings.res || data.check.basin.warnings.res.length === 0"
													class="text-body-2"
												>
													<i>None</i>
												</li>
										...
										<v-table small density="compact" class="my-2 transparent">
											<tbody>
												<tr>
													<th>Sediment</th>
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.avgTrappingEfficiencies.sediment, 3) }}
													</td>
												</tr>
												<tr>
													<th>Nitrogen</th>
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.avgTrappingEfficiencies.nitrogen, 3) }}
													</td>
												</tr>
												<tr>
													<th>Phosphorus</th>
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.avgTrappingEfficiencies.phosphorus, 3) }}
													</td>
												</tr>
											</tbody>
										</v-table>
										...
										<v-table small density="compact" class="my-2 transparent">
											<tbody>
												<tr>
													...
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.avgWaterLosses.totalRemoved, 3) }}
													</td>
												</tr>
												<tr>
													<th>Evaporation</th>
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.avgWaterLosses.evaporation, 3) }}
													</td>
												</tr>
												<tr>
													<th>Seepage</th>
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.avgWaterLosses.seepage, 3) }}
													</td>
												</tr>
											</tbody>
										</v-table>
										...
										<v-table small density="compact" class="my-2 transparent">
											<tbody>
												<tr>
													...
													<td class="text-right">
														{{ formatters.toNumberFormat(data.check.basin.avgReservoirTrends.numberReservoirs, 0) }}
													</td>
												</tr>
												<tr>
													<th>Final/Initial Volume (Max)</th>
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.avgReservoirTrends.maxVolume, 3) }}
													</td>
												</tr>
												<tr>
													<th>Final/Initial Volume (Min)</th>
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.avgReservoirTrends.minVolume, 3) }}
													</td>
												</tr>
												<tr>
													<th>Fraction of Period Empty (Max)</th>
													<td class="text-right">
														{{ formatters.toNumberDecimals(data.check.basin.avgReservoirTrends.fractionEmpty, 3) }}
													</td>
												</tr>
											</tbody>
										</v-table>
								...
								<v-table small density="compact">
									...
									<tbody>
										<tr v-for="(m, i) in data.check.basin.reservoirRows" :key="i">
											<td>{{ m.id }}</td>
											<td>
												{{ formatters.toNumberDecimals(m.sediment, 3) }}
											</td>
											<td>
												{{ formatters.toNumberDecimals(m.phosphorus, 3) }}
											</td>
											<td>
												{{ formatters.toNumberDecimals(m.nitrogen, 3) }}
											</td>
											<td>
												{{ formatters.toNumberDecimals(m.volumeRatio, 3) }}
											</td>
											<td>
												{{ formatters.toNumberDecimals(m.fractionEmpty, 3) }}
											</td>
											<td>
												{{ formatters.toNumberDecimals(m.seepage, 3) }}
											</td>
											<td>
												{{ formatters.toNumberDecimals(m.evapLoss, 3) }}
											</td>
										</tr>
									</tbody>
								</v-table>
					...
					<v-card>
						<v-card-title>
							<div class="d-flex align-center px-3 pt-2">
								<div>Channel Delivery Ratio</div>
								<v-spacer></v-spacer>
								<v-text-field
						...
						<v-card-item>
							<v-data-table
								...
								fixed-header
								height="500px"
								:items="data.check.basin.reachReport"
								:items-per-page="100"
								:headers="[
									{ key: 'id', title: 'Channel' },
									{ key: 'sediment', title: 'Sediment' },
									{ key: 'phosphorus', title: 'Phosphorus' },
									{ key: 'nitrogen', title: 'Nitrogen' },
								]"
								v-model:search="data.modals.reachReportFilter"
								:filter-keys="['id']"
							>
								...
								<template v-slot:item.sediment="{ value }">
									<div class="text-end">
										{{ formatters.toNumberDecimals(value, 3) }}
										%
									</div>
								</template>
				...
				<action-bar full-width>
					<v-btn variant="flat" @click="nextTab(-1)" class="border mr-2" :disabled="data.page.tabIndex == 0" title="Previous tab"
